...
| `exclude` | Method | Remove items from distribution that are named in keyword arguments  For example… |
| `exclude_package` | Method | Remove packages, modules, and extensions in named package |
| `fetch_build_egg` | Method | Fetch an egg needed for building |
| `fetch_build_eggs` | Method | Resolve pre-setup requirements |
| `finalize_options` | Method | Allow plugins to apply arbitrary operations to the distribution. Each hook may… |
| `find_config_files` | Method | Find as many configuration files as should be processed for this platform, and… |
| `get_cmdline_options` | Method | Return a '{cmd: {opt:val}}' map of all command-line options  Option names are a… |
| `get_command_class` | Method | Pluggable version of get_command_class() |
| `get_command_list` | Method | Get a list of (command, description) tuples. The list is divided into "standard… |
| `get_command_obj` | Method | Return the command object for 'command'.  Normally this object is cached on a p… |
| `get_command_packages` | Method | Return a list of packages from which commands are loaded. |
| `get_egg_cache_dir` | Method |  |
| `get_option_dict` | Method | Get the option dictionary for a given command.  If that command's option dictio… |
| `handle_display_options` | Method | If there were any non-global "display-only" options (--help-commands or the met… |
| `has_c_libraries` | Method |  |
| `has_contents_for` | Method | Return true if 'exclude_package(package)' would do something |
| `has_data_files` | Method |  |
| `has_ext_modules` | Method |  |
| `has_headers` | Method |  |
| `has_modules` | Method |  |
| `has_pure_modules` | Method |  |
| `has_scripts` | Method |  |
| `include` | Method | Add items to distribution that are named in keyword arguments  For example, 'di… |
| `is_pure` | Method |  |
| `iter_distribution_names` | Method | Yield all packages, modules, and extension names in distribution |
| `parse_command_line` | Method | Parse the setup script's command line, taken from the 'script_args' instance at… |
| `parse_config_files` | Method | Parses configuration files from various levels and loads configuration. |
| `print_command_list` | Method | Print a subset of the list of all commands -- used by 'print_commands()'. |
| `Extension` | Class | Describes a single extension module.  This means that all source files will be… |
| `PEP420PackageFinder` | Class | Generate a list of all Python packages found within a directory |
| `find` | Method | Return a list of all Python items (packages or modules, depending on the finder… |
| `PackageFinder` | Class | Generate a list of all Python packages found within a directory |
| `find` | Method | Return a list of all Python items (packages or modules, depending on the finder… |
| `Require` | Class | A prerequisite to building or installing a distribution |
| `full_name` | Method | Return full package/distribution name, w/version |
| `get_version` | Method | Get version number of installed module, 'None', or 'default'  Search 'paths' fo… |
| `is_current` | Method | Return true if dependency is present and up-to-date on 'paths' |
| `is_present` | Method | Return true if dependency is present on 'paths' |
| `version_ok` | Method | Is 'version' sufficiently up-to-date? |
| `SetuptoolsDeprecationWarning` | Class | Base class for warning deprecations in ``setuptools``  This class is not derive… |
| `emit` | Method | Private: reserved for ``setuptools`` internal use only |
| `findall` | Function | Find all files under 'dir' and return the list of full filenames. Unless dir is… |
| `setup` | Function | The gateway to the Distutils: do everything your setup script needs to do, in a… |
| `sic` | Class | Treat this string as-is (https://en.wikipedia.org/wiki/Sic) |
| `UnrecognizedFormat` | Class | Couldn't recognize the archive type |
| `default_filter` | Function | The default progress/filter callback; returns True for all files |
| `ensure_directory` | Function | Ensure that the parent directory of `path` exists |
| `unpack_archive` | Function | Unpack `filename` to `extract_dir`, or raise ``UnrecognizedFormat``  `progress_… |
| `unpack_directory` | Function | "Unpack" a directory, using the same interface as for archives  Raises ``Unreco… |
| `unpack_tarfile` | Function | Unpack tar/tar.gz/tar.bz2 `filename` to `extract_dir`  Raises ``UnrecognizedFor… |
| `unpack_zipfile` | Function | Unpack zip `filename` to `extract_dir`  Raises ``UnrecognizedFormat`` if `filen… |
| `Distribution` | Class | Distribution with support for tests and package data  This is an enhanced versi… |
| `announce` | Method |  |
| `dump_option_dicts` | Method |  |
| `exclude` | Method | Remove items from distribution that are named in keyword arguments  For example… |
...
| `parse_command_line` | Method | Parse the setup script's command line, taken from the 'script_args' instance at… |
| `parse_config_files` | Method | Parses configuration files from various levels and loads configuration. |
| `patch` | Method | Replace distutils.dist.Distribution with this class for the duration of this co… |
| `SetupRequirementsError` | Class | Common base class for all exceptions |
| `SetuptoolsDeprecationWarning` | Class | Base class for warning deprecations in ``setuptools``  This class is not derive… |
| `emit` | Method | Private: reserved for ``setuptools`` internal use only |
| `no_install_setup_requires` | Function | Temporarily disable installing setup_requires  Under PEP 517, the backend repor… |
| `parse_strings` | Function | Yield requirement strings for each specification in `strs`.  `strs` must be a s… |
| `same_path` | Function | Differs from os.path.samefile because it does not require paths to exist. Purel… |
| `suppress_known_deprecation` | Function |  |
| `alias` | Class | Define a shortcut that invokes one or more commands |
| `announce` | Method |  |
| `copy_file` | Method | Copy a file respecting verbose, dry-run and force flags.  (The former two defau… |
| `copy_tree` | Method | Copy an entire directory tree respecting verbose, dry-run, and force flags. |
| `debug_print` | Method | Print 'msg' to stdout if the global DEBUG (taken from the DISTUTILS_DEBUG envir… |
...
| `spawn` | Method | Spawn an external command respecting dry-run flag. |
| `warn` | Method |  |
| `config_file` | Function | Get the filename of the distutils, local, global, or per-user config  `kind` mu… |
| `edit_config` | Function | Edit a configuration file to include `settings`  `settings` is a dictionary of… |
| `format_alias` | Function |  |
| `option_base` | Class | Abstract base class for commands that mess with config files |
| `announce` | Method |  |
| `copy_file` | Method | Copy a file respecting verbose, dry-run and force flags.  (The former two defau… |
| `copy_tree` | Method | Copy an entire directory tree respecting verbose, dry-run, and force flags. |
| `debug_print` | Method | Print 'msg' to stdout if the global DEBUG (taken from the DISTUTILS_DEBUG envir… |
...
| `spawn` | Method | Spawn an external command respecting dry-run flag. |
| `warn` | Method |  |
| `shquote` | Function | Quote an argument for later parsing by shlex.split() |
| `Command` | Class | Setuptools internal actions are organized using a *command design pattern*. Thi… |
| `announce` | Method |  |
| `copy_file` | Method | Copy a file respecting verbose, dry-run and force flags.  (The former two defau… |
| `copy_tree` | Method | Copy an entire directory tree respecting verbose, dry-run, and force flags. |

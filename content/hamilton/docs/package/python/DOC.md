...
metadata:
  ...
  source: maintainer
  tags: "hamilton,python,mechanics,gui,legacy,pydy,GUI-Hamilton,Version-Sensitive,Driver,HamiltonDriver,parameterize,extract_columns,extract_fields,FunctionRegistry,H5pyDataLoader,ParquetDataLoader,CSVDataLoader,visualize_execution"
---
...
## API surface — hamilton (legacy 0.1.0) public API

```python
from hamilton.driver import Driver
from hamilton.function_modifiers import config, parameterize, extract_columns, extract_fields
from hamilton.io import H5pyDataLoader, ParquetDataLoader, CSVDataLoader
from hamilton.dataflows import import_module_from_url

class HamiltonDriver:
    def __init__(self, config, *modules): pass
    def execute(self, final_vars, overrides=None): pass
    def list_available_variables(self): pass
    def display_all_functions(self, output_file_path): pass
    def what_is_downstream_of(self, var_name): pass
    def what_is_upstream_of(self, var_name): pass
    def visualize_execution(self, final_vars, output_file_path): pass

class FunctionRegistry:
    def register(self, fn): pass
    def get(self, name): pass

driver = HamiltonDriver({"config_key": "value"}, my_module)
result = driver.execute(["output_a", "output_b"])
all_vars = driver.list_available_variables()
upstream = driver.what_is_upstream_of("final_output")
downstream = driver.what_is_downstream_of("input_data")
```

```python
class ConfigOverrides: pass
class ParameterizedNode: pass
class ColumnExtractor: pass
class FieldExtractor: pass
class HamiltonModule: pass
class DownstreamFinder: pass
class UpstreamFinder: pass
class GraphVisualizer: pass
class ExecutionPlan: pass
class NodeMetadata: pass
```

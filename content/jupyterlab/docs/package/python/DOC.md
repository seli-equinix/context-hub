...
| `LabApp` | Class | A Mixin class for shimming configuration from NotebookApp to ServerApp. This cl… |
| `add_traits` | Method | Dynamically add trait attributes to the HasTraits instance. |
| `class_config_rst_doc` | Method | Generate rST documentation for this class' config options.  Excludes traits def… |
| `class_config_section` | Method | Get the config section for this class.  Parameters ---------- classes : list, o… |
| `class_get_help` | Method | Get the help string for this class in ReST format.  If `inst` is given, its cur… |
...
| `has_trait` | Method | Returns True if the object has a trait with the specified name. |
| `hold_trait_notifications` | Method | Context manager for bundling trait change notifications and cross validation.… |
| `LogErrorHandler` | Class | A handler that exits with 1 on a logged error. |
| `acquire` | Method | Acquire the I/O thread lock. |
| `addFilter` | Method | Add the specified filter to this handler. |
| `close` | Method | Tidy up any resources used by the handler.  This version removes the handler fr… |
| `createLock` | Method | Acquire a thread lock for serializing access to the underlying I/O. |
| `emit` | Method | Emit a record.  If a formatter is specified, it is used to format the record. T… |
| `filter` | Method | Determine if a record is loggable by consulting all the filters.  The default i… |
| `flush` | Method | Flushes the stream. |
| `format` | Method | Format the specified record.  If a formatter is set, use it. Otherwise, use the… |
| `get_name` | Method |  |
| `handle` | Method | Conditionally emit the specified logging record.  Emission depends on filters w… |
| `handleError` | Method | Handle errors which occur during an emit() call.  This method should be called… |
| `release` | Method | Release the I/O thread lock. |
| `removeFilter` | Method | Remove the specified filter from this handler. |
| `setFormatter` | Method | Set the formatter for this handler. |
| `setLevel` | Method | Set the logging level of this handler.  level must be an int or a str. |
| `setStream` | Method | Sets the StreamHandler's stream to the specified value, if it is different.  Re… |
| `set_name` | Method |  |
| `TestEnv` | Class | Set Jupyter path variables to a temporary directory  Useful as a context manage… |
| `start` | Method |  |
| `stop` | Method |  |
| `get_app_dir` | Function | Get the configured JupyterLab app directory. |
| `run_async_process` | Function | Run an asynchronous command |
| `run_browser` | Function | Run the browser test and return an exit code. |
| `run_browser_sync` | Function | Run the browser test and return an exit code. |
| `run_test` | Function | Synchronous entry point to run a test function. func is a function that accepts… |
| `run_test_async` | Function | Run a test against the application. func is a function that accepts an app url… |
| `AppOptions` | Class | Options object for build system |
| `add_traits` | Method | Dynamically add trait attributes to the HasTraits instance. |
| `class_own_trait_events` | Method | Get a dict of all event handlers defined on this class, not a parent.  Works li… |
| `class_own_traits` | Method | Get a dict of all the traitlets defined on this class, not a parent.  Works lik… |
| `class_trait_names` | Method | Get a list of all the names of this class' traits.  This method is just like th… |
...
| `has_trait` | Method | Returns True if the object has a trait with the specified name. |
| `hold_trait_notifications` | Method | Context manager for bundling trait change notifications and cross validation.… |
| `notify_change` | Method | Notify observers of a change event |
| `observe` | Method | Setup a handler to be called when a trait changes.  This is used to setup dynam… |
| `on_trait_change` | Method | DEPRECATED: Setup a handler to be called when a trait changes.  This is used to… |
| `set_trait` | Method | Forcibly sets trait attribute, including read-only attributes. |
| `setup_instance` | Method | This is called **before** self.__init__ is called. |
| `trait_defaults` | Method | Return a trait's default value or a dictionary of them  Notes ----- Dynamically… |
| `trait_events` | Method | Get a ``dict`` of all the event handlers of this class.  Parameters ----------… |
| `trait_has_value` | Method | Returns True if the specified trait has a value.  This will return false even i… |
| `trait_metadata` | Method | Get metadata values for trait by key. |
| `trait_names` | Method | Get a list of all the names of this class' traits. |
| `trait_values` | Method | A ``dict`` of trait names and their values.  The metadata kwargs allow function… |
| `traits` | Method | Get a ``dict`` of all the traits of this class.  The dictionary is keyed on the… |
| `unobserve` | Method | Remove a trait change handler.  This is used to unregister handlers to trait ch… |
| `unobserve_all` | Method | Remove trait change handlers of any type for the specified name. If name is not… |
| `CoreConfig` | Class | An object representing a core config.  This enables custom lab application to o… |
| `add` | Method | Remove an extension/singleton.  If neither extension or mimeExtension is True (… |
| `clear_packages` | Method | Clear the packages/extensions. |
| `remove` | Method | Remove a package/extension.  name: string     The npm package name |
| `LockStatus` | Class | LockStatus(entire_extension_locked: bool, locked_plugins: Optional[frozenset[st… |
| `Process` | Class | A wrapper for a child process. |
| `get_log` | Method | Get our logger. |
| `terminate` | Method | Terminate the process and return the exit code. |
| `wait` | Method | Wait for the process to finish.  Returns ------- The process exit code. |
| `wait_async` | Method | Asynchronously wait for the process to finish. |
| `ProgressProcess` | Class | A wrapper for a child process. |
| `get_log` | Method | Get our logger. |
| `terminate` | Method | Terminate the process and return the exit code. |
| `wait` | Method | Wait for the process to finish.  Returns ------- The process exit code. |
| `wait_async` | Method | Asynchronously wait for the process to finish. |
...
| `parse_range` | Method |  |
| `test` | Method |  |
| `WatchHelper` | Class | A process helper for a watch process. |
| `get_log` | Method | Get our logger. |
| `terminate` | Method | Terminate the process. |
| `wait` | Method | Wait for the process to finish.  Returns ------- The process exit code. |
| `wait_async` | Method | Asynchronously wait for the process to finish. |
| `build` | Function | Build the JupyterLab application. |
| `build_check` | Function | Determine whether JupyterLab should be built.  Returns a list of messages. |
| `check_extension` | Function | Check if a JupyterLab extension is enabled or disabled. |
| `clean` | Function | Clean the JupyterLab application directory. |
| `dedupe_yarn` | Function | `yarn-deduplicate` with the `fewer` strategy to minimize total packages install… |
| `disable_extension` | Function | Disable a JupyterLab extension/plugin.  Returns `True` if a rebuild is recommen… |
| `enable_extension` | Function | Enable a JupyterLab extension/plugin.  Returns `True` if a rebuild is recommend… |
| `ensure_app` | Function | Ensure that an application directory is available.  If it does not exist, retur… |
| `ensure_core` | Function | Ensure that the core assets are available. |
| `ensure_dev` | Function | Ensure that the dev assets are available. |
| `ensure_node_modules` | Function | Ensure that node_modules is up to date.  Returns true if the node_modules was u… |
| `get_allowed_levels` | Function | Returns the levels where configs can be stored. |
| `get_app_dir` | Function | Get the configured JupyterLab app directory. |
| `get_app_info` | Function | Get a dictionary of information about the app. |
| `get_app_version` | Function | Get the application version. |
| `get_federated_extensions` | Function | Get the metadata about federated extensions |
| `get_latest_compatible_package_versions` | Function | Get the latest compatible version of a list of packages. |
| `get_package_url` | Function | Get the url from the extension data |
| `get_page_config` | Function | Get the page config for the application handler |
| `get_static_page_config` | Function | Get the static page config for JupyterLab  Parameters ---------- logger: logger… |
| `get_user_settings_dir` | Function | Get the configured JupyterLab user settings directory. |
| `get_workspaces_dir` | Function | Get the configured JupyterLab workspaces directory. |
| `gt` | Function |  |
| `gte` | Function |  |
| `install_extension` | Function | Install an extension package into JupyterLab.  The extension is first validated… |
| `link_package` | Function | Link a package against the JupyterLab build.  Returns `True` if a rebuild is re… |
| `list2cmdline` | Function | Shim for list2cmdline on posix. |
| `list_extensions` | Function | List the extensions. |
| `lock_extension` | Function | Lock a JupyterLab extension/plugin. |
| `lt` | Function |  |
| `lte` | Function |  |
| `make_semver` | Function |  |
| `pjoin` | Function | Join paths to create a real path. |
| `read_package` | Function | Read the package data in a given target tarball. |
| `uninstall_extension` | Function | Uninstall an extension by name or path.  Returns `True` if a rebuild is recomme… |
| `unlink_package` | Function | Unlink a package from JupyterLab by path or name.  Returns `True` if a rebuild… |
| `unlock_extension` | Function | Unlock a JupyterLab extension/plugin. |
| `update_extension` | Function | Update an extension by name, or all extensions. Either `name` must be given as… |
| `watch` | Function | Watch the application.  Parameters ---------- app_options: :class:`AppOptions`,… |
| `watch_dev` | Function | Run watch mode in a given directory.  Parameters ---------- logger: :class:`~lo… |
| `watch_packages` | Function | Run watch mode for the source packages.  Parameters ---------- logger: :class:`… |
| `which` | Function | Get the full path to a command.  Parameters ---------- command: str     The com… |
| `write_page_config` | Function | Write page config to disk |
| `CoreConfig` | Class | An object representing a core config.  This enables custom lab application to o… |
| `add` | Method | Remove an extension/singleton.  If neither extension or mimeExtension is True (… |
| `clear_packages` | Method | Clear the packages/extensions. |
| `remove` | Method | Remove a package/extension.  name: string     The npm package name |
| `pjoin` | Function | Join paths to create a real path. |
| `DebugLogFileMixin` | Class | The base class for all classes that have descriptors. |
| `add_traits` | Method | Dynamically add trait attributes to the HasTraits instance. |
| `class_config_rst_doc` | Method | Generate rST documentation for this class' config options.  Excludes traits def… |
| `class_config_section` | Method | Get the config section for this class.  Parameters ---------- classes : list, o… |
| `class_get_help` | Method | Get the help string for this class in ReST format.  If `inst` is given, its cur… |

...
| `prepare_metadata` | Method | Prepare metadata and store in the out_dir. Some backends doesn't provide that A… |
| `subprocess_runner` | Method |  |
| `LoggerWrapper` | Class | Read messages from a pipe and redirect them to a logger (see python's logging m… |
| `fileno` | Method |  |
| `getName` | Method | Return a string used for identification purposes only.  This method is deprecat… |
| `isDaemon` | Method | Return whether this thread is a daemon.  This method is deprecated, use the dae… |
| `is_alive` | Method | Return whether the thread is alive.  This method returns True just before the r… |
| `join` | Method | Wait until the thread terminates.  This blocks the calling thread until the thr… |
| `remove_newline` | Method |  |
| `run` | Method | Method representing the thread's activity.  You may override this method in a s… |
| `setDaemon` | Method | Set whether this thread is a daemon.  This method is deprecated, use the .daemo… |
| `setName` | Method | Set the name string for this thread.  This method is deprecated, use the name a… |
| `start` | Method | Start the thread's activity.  It must be called at most once per thread object.… |
| `stop` | Method |  |
| `PythonEnvironment` | Class | A project environment that is directly derived from a Python interpreter |
| `get_finder` | Method | Return the package finder of given index sources.  :param sources: a list of so… |
| `get_paths` | Method | Get paths like ``sysconfig.get_paths()`` for installation.  :param dist_name: T… |
| `get_working_set` | Method | Get the working set based on local packages directory. |
| `which` | Method | Get the full path of the given executable against this environment. |
| `Requirement` | Class | Base class of a package requirement. A requirement is a (virtual) specification… |
| `as_line` | Method |  |
| `as_pinned_version` | Method | Return a new requirement with the given pinned version. |
| `create` | Method |  |
| `from_dist` | Method |  |
| `from_pkg_requirement` | Method |  |
| `from_req_dict` | Method |  |
| `identify` | Method |  |
| `matches` | Method | Return whether the passed in PEP 508 string is the same requirement as this one. |
| `WorkingSet` | Class | A dictionary of currently installed distributions |
...
| `build_error` | Function | Get a build error with meaningful error message from the subprocess output. |
| `get_sys_config_paths` | Function | Return the sys_config.get_paths() result for the python interpreter |
| `log_subprocessor` | Function |  |
| `parse_requirement` | Function |  |
| `wrap_error` | Function |  |
| `EditableBuilder` | Class | Build egg-info in isolated env with managed Python. |
| `build` | Method | Build and store the artifact in out_dir, return the absolute path of the built… |
| `check_requirements` | Method |  |
...
| `subprocess_runner` | Method |  |
| `wrap_error` | Function |  |
| `BareEnvironment` | Class | Bare environment that does not depend on project files. |
| `get_finder` | Method | Return the package finder of given index sources.  :param sources: a list of so… |
| `get_paths` | Method | Get paths like ``sysconfig.get_paths()`` for installation.  :param dist_name: T… |
| `get_working_set` | Method | Get the working set based on local packages directory. |
| `which` | Method | Get the full path of the given executable against this environment. |
| `Candidate` | Class | A concrete candidate that can be downloaded and installed. A candidate comes fr… |
| `as_lockfile_entry` | Method | Build a lockfile entry dictionary for the candidate. |
| `copy_with` | Method |  |
| `format` | Method | Format for output. |
| `from_installation_candidate` | Method | Build a candidate from unearth's find result. |
| `get_revision` | Method |  |
| `identify` | Method |  |
| `prepare` | Method | Prepare the candidate for installation. |
| `EnvSpec` | Class | EnvSpec(requires_python: 'VersionSpecifier', platform: 'Platform \| None' = None… |
| `as_dict` | Method |  |
| `compare` | Method |  |
| `compatibility` | Method |  |
| `current` | Method |  |
| `from_marker` | Method | Create an EnvSpec from a Marker object. |
| `from_spec` | Method |  |
| `is_allow_all` | Method |  |
| `markers` | Method |  |
| `markers_with_defaults` | Method |  |
| `markers_with_python` | Method |  |
| `replace` | Method |  |
| `wheel_compatibility` | Method |  |
...
| `validate` | Method |  |
| `HookManager` | Class |  |
| `should_run` | Method | Tells whether a task given its name should run or not according to the current… |
| `skipping` | Method | Temporarily skip some hooks. |
| `try_emit` | Method | Emit a hook signal if rules allow it. |
| `LockedRepository` | Class | A Repository acts as the source of packages and metadata. |
| `add_package` | Method |  |
| `dependency_generators` | Method | Return an iterable of getter functions to get dependencies, which will be calle… |
| `evaluate_candidates` | Method |  |
| `fetch_hashes` | Method | Fetch hashes for candidates in parallel |
| `find_candidates` | Method | Find candidates of the given NamedRequirement. Let it to be implemented in subc… |
| `get_dependencies` | Method | Get (dependencies, python_specifier, summary) of the candidate. |
| `get_filtered_sources` | Method | Get matching sources based on the index attribute. |
| `get_hashes` | Method | Get hashes of all possible installable candidates of a given package version. |
| `is_this_package` | Method | Whether the requirement is the same as this package |
| `make_this_candidate` | Method | Make a candidate for this package. In this case the finder will look for a cand… |
| `merge_result` | Method |  |
| `search` | Method | Search package by name or summary.  :param query: query string :returns: search… |
| `Package` | Class | Package(candidate: 'Candidate', dependencies: 'list[str] \| None' = None, summar… |
| `PdmException` | Class | Common base class for all non-exit exceptions. |
| `PdmUsageError` | Class | Common base class for all non-exit exceptions. |
| `Project` | Class | Core project class.  Args:     core: The core instance.     root_path: The root… |
| `add_dependencies` | Method | Add requirements to the given group, and return the requirements of that group. |
| `cache` | Method |  |
| `env_or_setting` | Method | Get a value from environment variable and fallback on a given setting.  Returns… |
| `find_interpreters` | Method | Return an iterable of interpreter paths that matches the given specifier, which… |
| `get_best_matching_cpython_version` | Method | Returns the best matching CPython version that fits requires-python, this platf… |
| `get_dependencies` | Method |  |
| `get_environment` | Method |  |
| `get_locked_repository` | Method |  |
| `get_provider` | Method | Build a provider class for resolver.  :param strategy: the resolve strategy :pa… |
| `get_reporter` | Method | Return the reporter object to construct a resolver.  :param requirements: requi… |
| `get_repository` | Method | Get the repository object |
| `get_resolver` | Method | Get the resolver class to use for the project. |
| `get_setting` | Method | Get a setting from its dotted key (without the `tool.pdm` prefix).  Returns `No… |
| `get_sources` | Method |  |
| `get_synchronizer` | Method | Get the synchronizer class to use for the project. |
| `init_global_project` | Method |  |
| `is_lockfile_hash_match` | Method |  |

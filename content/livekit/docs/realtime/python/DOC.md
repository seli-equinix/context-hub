...
| `aclose` | Method | Asynchronously close the audio stream.  This method cleans up resources associa… |
| `from_participant` | Method | Create an `AudioStream` from a participant's audio track.  Args:     participan… |
| `from_track` | Method | Create an `AudioStream` from an existing audio track.  Args:     track (Track):… |
| `ByteStreamInfo` | Class | ByteStreamInfo(stream_id: 'str', mime_type: 'str', topic: 'str', timestamp: 'in… |
| `ByteStreamReader` | Class |  |
| `ByteStreamWriter` | Class |  |
| `aclose` | Method |  |
| `write` | Method |  |
| `ConnectError` | Class | Common base class for all non-exit exceptions. |
| `DataPacket` | Class | DataPacket(data: 'bytes', kind: 'proto_room.DataPacketKind.ValueType', particip… |
| `DataTrackFrame` | Class | A frame published on a data track, consisting of a payload and optional metadat… |
| `DataTrackInfo` | Class | Information about a published data track. |
| `DataTrackStream` | Class | An active subscription to a remote data track.  Use as an async iterator to rec… |
| `aclose` | Method |  |
| `close` | Method | Explicitly close the subscription and unsubscribe. |
| `read` | Method | Read a single frame, or ``None`` if the stream has ended. |
| `E2EEManager` | Class |  |
| `frame_cryptors` | Method | Retrieves the list of frame cryptors for participants.  Returns:     List[Frame… |
| `set_enabled` | Method | Enables or disables end-to-end encryption.  Parameters:     enabled (bool): Tru… |
| `E2EEOptions` | Class | E2EEOptions(key_provider_options: livekit.rtc.e2ee.KeyProviderOptions = <factor… |
| `EventEmitter` | Class | Abstract base class for generic types.  On Python 3.12 and newer, generic class… |
| `emit` | Method | Trigger all callbacks associated with the given event.  Args:     event (T): Th… |
| `off` | Method | Unregister a callback from an event.  Args:     event (T): The event to stop li… |
| `on` | Method | Register a callback to be called whenever the event is emitted.  If a callback… |
| `once` | Method | Register a callback to be called only once when the event is emitted.  If a cal… |
| `FrameCryptor` | Class |  |
| `set_enabled` | Method | Enables or disables frame encryption.  Parameters:     enabled (bool): True to… |
| `set_key_index` | Method | Sets the key index for encryption/decryption.  Parameters:     key_index (int):… |
| `FrameProcessor` | Class | Abstract base class for generic types.  On Python 3.12 and newer, generic class… |
| `KeyProvider` | Class |  |
| `export_key` | Method | Exports the encryption key for a specific participant.  Parameters:     partici… |
| `export_shared_key` | Method | Exports the shared encryption key.  Parameters:     key_index (int): The index… |
| `ratchet_key` | Method | Ratchets the encryption key for a specific participant to a new key.  Parameter… |
| `ratchet_shared_key` | Method | Ratchets the shared encryption key to a new key.  Parameters:     key_index (in… |
| `set_key` | Method | Sets the encryption key for a specific participant.  Parameters:     participan… |
| `set_shared_key` | Method | Sets the shared encryption key.  Parameters:     key (bytes): The new shared ke… |
| `KeyProviderOptions` | Class | KeyProviderOptions(shared_key: Optional[bytes] = None, ratchet_salt: bytes = b'… |
| `LocalAudioTrack` | Class |  |
| `create_audio_track` | Method |  |
| `get_stats` | Method |  |
| `mute` | Method |  |
| `unmute` | Method |  |
| `LocalDataTrack` | Class | Data track published by the local participant. |
| `is_published` | Method | Whether or not the track is still published. |
| `try_push` | Method | Try pushing a frame to subscribers of the track.  See :class:`DataTrackFrame` f… |
| `unpublish` | Method | Unpublishes the track. |
| `LocalParticipant` | Class | Represents the local participant in a room. |
| `perform_rpc` | Method | Initiate an RPC call to a remote participant.  Args:     destination_identity (… |
| `publish_data` | Method | Publish arbitrary data to the room.  Args:     payload (Union[bytes, str]): The… |
| `publish_data_track` | Method | Publishes a data track.  Args:     name: The track name used to identify the tr… |
| `publish_dtmf` | Method | Publish SIP DTMF message.  Args:     code (int): DTMF code.     digit (str): DT… |
| `publish_track` | Method | Publish a local track to the room.  Args:     track (LocalTrack): The track to… |
| `publish_transcription` | Method | Publish transcription data to the room.  Args:     transcription (Transcription… |
| `register_rpc_method` | Method | Establishes the participant as a receiver for calls of the specified RPC method… |
| `send_file` | Method |  |
| `send_text` | Method |  |
| `set_attributes` | Method | Set custom attributes for the local participant.  Note: this requires `canUpdat… |
| `set_metadata` | Method | Set the metadata for the local participant.  Note: this requires `canUpdateOwnM… |
| `set_name` | Method | Set the name for the local participant.  Note: this requires `canUpdateOwnMetad… |
| `set_track_subscription_permissions` | Method | Set the track subscription permissions for the local participant.  Args:     al… |
| `stream_bytes` | Method | Returns a ByteStreamWriter that allows to write individual chunks of bytes to a… |
| `stream_text` | Method | Returns a TextStreamWriter that allows to write individual chunks of text to a… |
| `unpublish_track` | Method | Unpublish a track from the room.  Args:     track_sid (str): The SID of the tra… |
| `unregister_rpc_method` | Method | Unregisters a previously registered RPC method.  Args:     method (str): The na… |
| `LocalTrackPublication` | Class |  |
| `wait_for_subscription` | Method |  |
| `LocalVideoTrack` | Class |  |
| `create_video_track` | Method |  |
| `get_stats` | Method |  |
| `mute` | Method |  |
| `unmute` | Method |  |
| `MediaDevices` | Class | High-level interface to native audio devices.  - Device enumeration helpers. -… |
| `default_input_device` | Method | Return the default input device index (or None). |
| `default_output_device` | Method | Return the default output device index (or None). |
| `list_input_devices` | Method | List available input devices.  Returns a list of dictionaries with the `soundde… |
| `list_output_devices` | Method | List available output devices with indices. |
| `open_input` | Method | Open the default (or chosen) audio input device and start capture.  Frames are… |
| `open_output` | Method | Create an `OutputPlayer` for rendering and (optionally) AEC reverse.  If an inp… |
| `NoiseCancellationOptions` | Class | NoiseCancellationOptions(module_id: 'str', options: 'dict[str, Any]') |
| `Participant` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `PushFrameError` | Class | Frame could not be pushed to a data track.  Pushing a frame can fail for severa… |
| `RemoteAudioTrack` | Class |  |
| `get_stats` | Method |  |
| `RemoteDataTrack` | Class | Data track published by a remote participant. |
| `is_published` | Method | Whether or not the track is still published. |
| `subscribe` | Method | Subscribes to the data track to receive frames.  Args:     buffer_size: Maximum… |
| `RemoteParticipant` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `RemoteTrackPublication` | Class |  |
| `set_subscribed` | Method |  |
| `RemoteVideoTrack` | Class |  |
| `get_stats` | Method |  |
| `Room` | Class | Abstract base class for generic types.  On Python 3.12 and newer, generic class… |
| `connect` | Method | Connects to a LiveKit room using the specified URL and token.  Parameters:… |
| `disconnect` | Method | Disconnects from the room. |
| `emit` | Method | Trigger all callbacks associated with the given event.  Args:     event (T): Th… |
| `get_rtc_stats` | Method |  |
| `isconnected` | Method | Checks if the room is currently connected.  Returns:     bool: True if connecte… |
| `off` | Method | Unregister a callback from an event.  Args:     event (T): The event to stop li… |
| `on` | Method | Registers an event handler for a specific event type.  Parameters:     event (E… |
| `once` | Method | Register a callback to be called only once when the event is emitted.  If a cal… |
| `register_byte_stream_handler` | Method |  |
| `register_text_stream_handler` | Method |  |
| `unregister_byte_stream_handler` | Method |  |
| `unregister_text_stream_handler` | Method |  |
| `RoomOptions` | Class | RoomOptions(auto_subscribe: 'bool' = True, dynacast: 'bool' = False, e2ee: 'E2E… |
| `RpcError` | Class | Specialized error handling for RPC methods.  Instances of this type, when throw… |
| `RpcInvocationData` | Class | Data passed to method handler for incoming RPC invocations  Attributes:     req… |
| `RtcConfiguration` | Class | RtcConfiguration(ice_transport_type: 'proto_room.IceTransportType.ValueType' =… |
| `RtcStats` | Class | RtcStats(publisher_stats: 'list[proto_stats.RtcStats]', subscriber_stats: 'list… |
| `SipDTMF` | Class | SipDTMF(code: 'int', digit: 'str', participant: 'RemoteParticipant \| None' = No… |
| `SubscribeDataTrackError` | Class | An error that can occur when subscribing to a data track. |
| `TextStreamInfo` | Class | TextStreamInfo(stream_id: 'str', mime_type: 'str', topic: 'str', timestamp: 'in… |
| `TextStreamReader` | Class |  |
| `read_all` | Method |  |
| `TextStreamWriter` | Class |  |
| `aclose` | Method |  |
| `write` | Method |  |
| `Track` | Class |  |
| `get_stats` | Method |  |
| `TrackPublication` | Class |  |
| `Transcription` | Class | Transcription(participant_identity: str, track_sid: str, segments: List[Forward… |
| `TranscriptionSegment` | Class | TranscriptionSegment(id: str, text: str, start_time: int, end_time: int, langua… |
| `VideoFrame` | Class | Represents a video frame with associated metadata and pixel data.  This class p… |
| `convert` | Method | Converts the current video frame to a different format type, optionally flippin… |
| `get_plane` | Method | Returns the memoryview of a specific plane in the video frame, based on its ind… |
| `VideoFrameEvent` | Class | VideoFrameEvent(frame: 'VideoFrame', timestamp_us: 'int', rotation: 'proto_vide… |
| `VideoSource` | Class |  |
| `aclose` | Method |  |
| `capture_frame` | Method |  |
| `VideoStream` | Class | VideoStream is a stream of video frames received from a RemoteTrack. |
| `aclose` | Method |  |
| `from_participant` | Method |  |
| `from_track` | Method |  |
| `combine_audio_frames` | Function | Combines one or more `rtc.AudioFrame` objects into a single `rtc.AudioFrame`.… |
| `AudioFrame` | Class | A class that represents a frame of audio data with specific properties such as… |
| `create` | Method | Create a new empty AudioFrame instance with specified sample rate, number of ch… |
| `to_wav_bytes` | Method | Convert the audio frame data to a WAV-formatted byte stream.  Returns:     byte… |
| `AudioProcessingModule` | Class | Provides WebRTC audio processing capabilities including echo cancellation, nois… |

...
| `delete_run` | Method | Delete a non-active run.  :param run_id: id of the run |
| `find_job_id_by_name` | Method | Find job id by its name; if there are multiple jobs with the same name, raise A… |
| `find_pipeline_id_by_name` | Method | Find pipeline id by its name; if multiple pipelines with the same name, raise A… |
| `get_cluster_state` | Method | Retrieve run state of the cluster.  :param cluster_id: id of the cluster :retur… |
| `get_conn` | Method | Return connection for the hook. |
| `get_connection` | Method | Get connection, given connection id.  :param conn_id: connection id :return: co… |
| `get_connection_form_widgets` | Method |  |
| `get_hook` | Method | Return default hook for this connection id.  :param conn_id: connection id :par… |
| `get_job_id` | Method | Retrieve job_id from run_id.  :param run_id: id of the run :return: Job id for… |
| `get_latest_repair_id` | Method | Get latest repair id if any exist for run_id else None. |
| `get_openlineage_database_dialect` | Method |  |
| `get_openlineage_database_info` | Method | Return Databricks-specific database info for OpenLineage namespace resolution. |
| `get_openlineage_default_schema` | Method |  |
| `get_repo_by_path` | Method | Obtain Repos ID by path.  :param path: path to a repository :return: Repos ID i… |
| `get_run` | Method | Retrieve run information.  :param run_id: id of the run :return: state of the r… |
| `get_run_output` | Method | Retrieve run output of the run.  :param run_id: id of the run :return: output o… |
| `get_run_page_url` | Method | Retrieve run_page_url.  :param run_id: id of the run :return: URL of the run pa… |
| `RunLifeCycleState` | Class | Enum for the run life cycle state concept of Databricks runs.  See more informa… |
| `RunState` | Class | Utility class for the run state concept of Databricks runs. |
| `from_json` | Method |  |
| `to_json` | Method |  |
| `SQLStatementState` | Class | Utility class for the SQL statement state concept of Databricks statements. |
| `from_json` | Method |  |
| `to_json` | Method |  |
| `BaseDatabricksHook` | Class | Base for interaction with Databricks.  :param databricks_conn_id: Reference to… |
| `aget_connection` | Method | Get connection (async), given connection id.  :param conn_id: connection id :re… |
...
| `get_ui_field_behaviour` | Method |  |
| `logger` | Method | Return a logger. |
| `BearerAuth` | Class | aiohttp only ships BasicAuth, for Bearer auth we need a subclass of BasicAuth. |
| `decode` | Method | Create a BasicAuth object from an Authorization HTTP header. |
| `encode` | Method | Encode credentials. |
| `from_url` | Method | Create BasicAuth from url. |
| `BaseDatabricksHook` | Class | Base for interaction with Databricks.  :param databricks_conn_id: Reference to… |
| `aget_connection` | Method | Get connection (async), given connection id.  :param conn_id: connection id :re… |
| `get_conn` | Method | Return connection for the hook. |
| `get_connection` | Method | Get connection, given connection id.  :param conn_id: connection id :return: co… |
...
| `DatabricksSqlExecutionTimeout` | Class | Raised when a sql execution times out. |
| `serialize` | Method |  |
| `DatabricksSqlHook` | Class | Hook to interact with Databricks SQL.  :param databricks_conn_id: Reference to… |
| `aget_connection` | Method | Get connection (async), given connection id.  :param conn_id: connection id :re… |
| `bulk_dump` | Method | Dump a database table into a tab-delimited file.  :param table: The name of the… |
| `bulk_load` | Method | Load a tab-delimited file into a database table.  :param table: The name of the… |
| `get_autocommit` | Method | Get autocommit setting for the provided connection.  :param conn: Connection to… |
| `get_conn` | Method | Return a Databricks SQL connection object. |
| `get_conn_id` | Method |  |
| `get_connection` | Method | Get connection, given connection id.  :param conn_id: connection id :return: co… |
| `get_connection_form_widgets` | Method |  |
| `get_cursor` | Method | Return a cursor. |
| `get_db_log_messages` | Method | Log all database messages sent to the client during the session.  :param conn:… |
| `get_df` | Method | Execute the sql and returns a dataframe.  :param sql: the sql statement to be e… |
| `get_df_by_chunks` | Method | Execute the sql and return a generator.  :param sql: the sql statement to be ex… |
| `get_first` | Method | Execute the sql and return the first resulting row.  :param sql: the sql statem… |
| `get_hook` | Method | Return default hook for this connection id.  :param conn_id: connection id :par… |
| `get_openlineage_authority_part` | Method | Get authority part from Airflow Connection.  The authority represents the hostn… |
| `get_openlineage_database_dialect` | Method | Return database dialect used for SQL parsing.  For a list of supported dialects… |
| `get_openlineage_database_info` | Method | Return database specific information needed to generate and parse lineage metad… |
| `get_openlineage_database_specific_lineage` | Method | Emit separate OpenLineage events for each Databricks query, based on executed q… |
| `get_openlineage_default_schema` | Method | Return default schema specific to database.  .. seealso::     - :class:`airflow… |
| `get_pandas_df` | Method | Execute the sql and returns a pandas dataframe.  :param sql: the sql statement… |
| `get_pandas_df_by_chunks` | Method |  |
| `get_records` | Method | Execute the sql and return a set of records.  :param sql: the sql statement to… |
| `get_sqlalchemy_engine` | Method | Get an sqlalchemy_engine object.  :param engine_kwargs: Kwargs used in :func:`~… |
| `get_table_schema` | Method | Return column names and types for a table using SQLAlchemy Inspector.  :param t… |
| `get_ui_field_behaviour` | Method |  |
| `get_uri` | Method | Extract the URI from the connection.  :return: the extracted uri. |
| `insert_rows` | Method | Insert a collection of tuples into a table.  Rows are inserted in chunks, each… |
| `logger` | Method | Return a logger. |
| `run` | Method | Run a command or a list of commands.  Pass a list of SQL statements to the SQL… |
| `set_autocommit` | Method | Set the autocommit flag on the connection. |
| `create_timeout_thread` | Function | Create a timeout timer that cancels the connection and sets a timeout flag. |
| `DatabricksCreateJobsOperator` | Class | Creates (or resets) a Databricks job using the API endpoint.  .. seealso::… |
| `add_inlets` | Method | Set inlets to this operator. |
| `add_outlets` | Method | Define the outlets of this operator. |
| `as_setup` | Method | Mark a task as setup task. |
| `as_teardown` | Method | Mark a task as teardown and set its setups as direct relatives. |
| `defer` | Method | Mark this Operator "deferred", suspending its execution until the provided trig… |
| `dry_run` | Method | Perform dry run for the operator - just render template fields. |
| `execute` | Method | Derive when creating an operator.  The main method to execute the task. Context… |
| `expand_start_trigger_args` | Method | Get the start_trigger_args value of the current abstract operator.  Since a Bas… |
| `get_closest_mapped_task_group` | Method | Get the mapped task group "closest" to this task in the Dag.  :meta private: |
| `get_dag` | Method |  |
| `get_direct_relative_ids` | Method | Get set of the direct relative ids to the current task, upstream or downstream. |
| `get_direct_relatives` | Method | Get list of the direct relatives to the current task, upstream or downstream. |
| `get_flat_relative_ids` | Method | Get a flat set of relative IDs, upstream or downstream.  Will recurse each rela… |
| `get_flat_relatives` | Method | Get a flat list of relatives, either upstream or downstream.  :param upstream:… |
| `get_needs_expansion` | Method | Return true if the task is MappedOperator or is in a mapped task group.  :meta… |
| `get_serialized_fields` | Method | Stringified Dags and operators contain exactly these fields. |
| `get_template_env` | Method | Get the template environment for rendering templates. |
| `get_upstreams_follow_setups` | Method | All upstreams and, for each upstream setup, its respective teardowns.  :param d… |
| `get_upstreams_only_setups` | Method | Return relevant upstream setups.  This method is meant to be used when we are c… |
| `get_upstreams_only_setups_and_teardowns` | Method | Only *relevant* upstream setups and their teardowns.  This method is meant to b… |
| `has_dag` | Method | Return True if the Operator has been assigned to a Dag. |
| `iter_mapped_dependants` | Method | Return mapped nodes that depend on the current task the expansion.  For now, th… |
| `iter_mapped_task_groups` | Method | Return mapped task groups this task belongs to.  Groups are returned from the i… |
| `on_kill` | Method | Override this method to clean up subprocesses when a task instance gets killed.… |
| `partial` | Method |  |
| `post_execute` | Method | Execute right after self.execute() is called.  It is passed the execution conte… |
| `pre_execute` | Method | Execute right before self.execute() is called. |
| `prepare_for_execution` | Method | Lock task for execution to disable custom action in ``__setattr__`` and return… |
| `prepare_template` | Method | Execute after the templated fields get replaced by their content.  If you need… |
| `render_template` | Method | Render a templated string.  If *content* is a collection holding multiple templ… |
| `DatabricksExecutionTrigger` | Class | The trigger handles the logic of async communication with DataBricks API.  :par… |
| `cleanup` | Method | Cleanup the trigger.  Called when the trigger is no longer needed, and it's bei… |
| `logger` | Method | Return a logger. |
| `on_kill` | Method | Cancel the Databricks run when the trigger is cancelled by a user action. |
| `repr` | Method |  |
| `run` | Method | Run the trigger in an asynchronous context.  The trigger should yield an Event… |
| `serialize` | Method | Return the information needed to reconstruct this Trigger.  :return: Tuple of (… |
| `DatabricksHook` | Class | Interact with Databricks.  :param databricks_conn_id: Reference to the :ref:`Da… |
| `a_get_cluster_state` | Method | Async version of `get_cluster_state`.  :param cluster_id: id of the cluster :re… |
| `a_get_run` | Method | Async version of `get_run`.  :param run_id: id of the run :return: state of the… |
| `a_get_run_output` | Method | Async version of `get_run_output()`.  :param run_id: id of the run :return: out… |
...
| `get_run_output` | Method | Retrieve run output of the run.  :param run_id: id of the run :return: output o… |
| `get_run_page_url` | Method | Retrieve run_page_url.  :param run_id: id of the run :return: URL of the run pa… |
| `DatabricksJobRunLink` | Class | Constructs a link to monitor a Databricks Job Run. |
| `get_link` | Method | Link to external system.  :param operator: The Airflow operator object this lin… |
| `DatabricksNotebookOperator` | Class | Runs a notebook on Databricks using an Airflow operator.  The DatabricksNoteboo… |
| `add_inlets` | Method | Set inlets to this operator. |
| `add_outlets` | Method | Define the outlets of this operator. |
| `as_setup` | Method | Mark a task as setup task. |
| `as_teardown` | Method | Mark a task as teardown and set its setups as direct relatives. |
| `defer` | Method | Mark this Operator "deferred", suspending its execution until the provided trig… |
| `dry_run` | Method | Perform dry run for the operator - just render template fields. |
| `execute` | Method | Execute the operator. Launch the job and monitor it if wait_for_termination is… |

...
| `construct_document` | Method |  |
| `construct_mapping` | Method |  |
| `construct_object` | Method |  |
| `construct_pairs` | Method |  |
| `construct_scalar` | Method |  |
| `display_notebook_help` | Function | Display help on notebook parameters.  Parameters ---------- ctx : click.Context… |
| `execute_notebook` | Function | Executes a single notebook locally.  Parameters ---------- input_path : str or… |
| `print_papermill_version` | Function |  |
| `read_yaml_file` | Function | Reads a YAML file from the location specified at 'path'. |
| `PapermillNotebookClient` | Class | Module containing a  that executes the code cells and updates outputs |
| `add_traits` | Method | Dynamically add trait attributes to the HasTraits instance. |
| `async_execute` | Method | Executes each code cell.  Parameters ---------- kwargs :     Any option for ``s… |
| `async_execute_cell` | Method | Executes a single code cell.  To execute all cells see :meth:`execute`.  Parame… |
| `async_setup_kernel` | Method | Context manager for setting up the kernel to execute a notebook.  This assigns… |
| `async_start_new_kernel` | Method | Creates a new kernel.  Parameters ---------- kwargs :     Any options for ``sel… |
| `async_start_new_kernel_client` | Method | Creates a new kernel client.  Returns ------- kc : KernelClient     Kernel clie… |
| `async_wait_for_reply` | Method | Wait for a message reply. |
...
| `clear_display_id_mapping` | Method | Clear a display id mapping for a cell. |
| `clear_output` | Method | Clear output. |
| `create_kernel_manager` | Method | Creates a new kernel manager.  Returns ------- km : KernelManager     Kernel ma… |
| `execute` | Method | Wraps the parent class process call slightly |
| `execute_cell` | Method | Executes a single code cell.  To execute all cells see :meth:`execute`.  Parame… |
| `handle_comm_msg` | Method | Handle a comm message. |
...
| `log_output_message` | Method | Process a given output. May log it in the configured logger and/or write it int… |
...
| `on_comm_open_jupyter_widget` | Method | Handle a jupyter widget comm open. |
| `on_trait_change` | Method | DEPRECATED: Setup a handler to be called when a trait changes.  This is used to… |
| `output` | Method | Handle output. |
| `Engine` | Class | Base class for engines.  Other specific engine classes should inherit and imple… |
| `execute_managed_notebook` | Method | An abstract method where implementation will be defined in a subclass. |
| `execute_notebook` | Method | A wrapper to handle notebook execution tasks.  Wraps the notebook object in a `… |
| `nb_kernel_name` | Method | Use default implementation to fetch kernel name from the notebook object |
| `nb_language` | Method | Use default implementation to fetch programming language from the notebook obje… |
| `NBClientEngine` | Class | A notebook engine representing an nbclient process.  This can execute a noteboo… |
| `execute_managed_notebook` | Method | Performs the actual execution of the parameterized notebook locally.  Args:… |
| `execute_notebook` | Method | A wrapper to handle notebook execution tasks.  Wraps the notebook object in a `… |
| `nb_kernel_name` | Method | Use default implementation to fetch kernel name from the notebook object |
| `nb_language` | Method | Use default implementation to fetch programming language from the notebook obje… |
| `NotebookExecutionManager` | Class | Wrapper for execution state of a notebook.  This class is a wrapper for noteboo… |
| `autosave_cell` | Method | Saves the notebook if it's been more than self.autosave_cell_every seconds sinc… |
| `cell_complete` | Method | Finalize metadata for a cell and save notebook.  Optionally called by engines d… |
| `cell_exception` | Method | Set metadata when an exception is raised.  Called by engines when an exception… |
| `cell_start` | Method | Set and save a cell's start state.  Optionally called by engines during executi… |
| `cleanup_pbar` | Method | Clean up a progress bar |
| `complete_pbar` | Method | Refresh progress bar |
| `get_cell_description` | Method | Fetches cell description if present |
| `notebook_complete` | Method | Finalize the metadata for a notebook and save the notebook to the output path.… |
| `notebook_start` | Method | Initialize a notebook, clearing its metadata, and save it.  When starting a not… |
| `now` | Method | Helper to return current UTC time |
| `save` | Method | Saves the wrapped notebook state.  If an output path is known, this triggers a… |
| `set_timer` | Method | Initializes the execution timer for the notebook.  This is called automatically… |
| `PapermillEngines` | Class | The holder which houses any engine registered with the system.  This object is… |
| `execute_notebook_with_engine` | Method | Fetch a named engine and execute the nb object against it. |
| `get_engine` | Method | Retrieves an engine by name. |
| `nb_kernel_name` | Method | Fetch kernel name from the document by dropping-down into the provided engine. |
| `nb_language` | Method | Fetch language from the document by dropping-down into the provided engine. |
| `register` | Method | Register a named engine |
| `register_entry_points` | Method | Register entrypoints for an engine  Load handlers provided by other packages |
| `PapermillException` | Class | Raised when an exception is encountered when operating on a notebook. |
| `PapermillNotebookClient` | Class | Module containing a  that executes the code cells and updates outputs |
| `add_traits` | Method | Dynamically add trait attributes to the HasTraits instance. |
| `async_execute` | Method | Executes each code cell.  Parameters ---------- kwargs :     Any option for ``s… |
...
| `on_trait_change` | Method | DEPRECATED: Setup a handler to be called when a trait changes.  This is used to… |
| `output` | Method | Handle output. |
| `catch_nb_assignment` | Function | Wrapper to catch `nb` keyword arguments  This helps catch `nb` keyword argument… |
| `merge_kwargs` | Function | Merge named argument.  Function takes a dictionary of caller arguments and call… |
| `nb_kernel_name` | Function | Helper for fetching out the kernel name from a notebook object.  Parameters ---… |
| `nb_language` | Function | Helper for fetching out the programming language from a notebook object.  Param… |
| `remove_args` | Function | Remove arguments from kwargs.  Parameters ---------- args : list     Argument n… |
| `write_ipynb` | Function | Saves a notebook object to the specified path. Args:     nb_node (nbformat.Note… |
| `AwsError` | Class | Raised when an AWS Exception is encountered. |
| `FileExistsError` | Class | Raised when a File already exists on S3. |
| `PapermillException` | Class | Raised when an exception is encountered when operating on a notebook. |
| `PapermillExecutionError` | Class | Raised when an exception is encountered in a notebook. |
| `PapermillMissingParameterException` | Class | Raised when a parameter without a value is required to operate on a notebook. |
| `PapermillOptionalDependencyException` | Class | Raised when an exception is encountered when an optional plugin is missing. |
| `PapermillParameterOverwriteWarning` | Class | Callee overwrites caller argument to pass down the stream. |
| `PapermillRateLimitException` | Class | Raised when an io request has been rate limited |
| `PapermillWarning` | Class | Base warning for papermill. |
| `missing_dependency_generator` | Function |  |
| `missing_environment_variable_generator` | Function |  |
| `strip_color` | Function | Remove most ANSI color and style sequences from a string  Based on https://pypi… |
| `PapermillExecutionError` | Class | Raised when an exception is encountered in a notebook. |
| `add_builtin_parameters` | Function | Add built-in parameters to a dictionary of parameters  Parameters ---------- pa… |
| `chdir` | Function | Change working directory to `path` and restore old path on exit.  `path` can be… |
| `execute_notebook` | Function | Executes a single notebook locally.  Parameters ---------- input_path : str or… |
| `get_pretty_path` | Function |  |
| `load_notebook_node` | Function | Returns a notebook object with papermill metadata loaded from the specified pat… |
| `local_file_io_cwd` | Function |  |
| `parameterize_notebook` | Function | Assigned parameters into the appropriate place in the input notebook  Parameter… |
| `parameterize_path` | Function | Format a path with a provided dictionary of parameters  Parameters ---------- p… |
| `prepare_notebook_metadata` | Function | Prepare metadata associated with a notebook and its cells  Parameters ---------… |
| `raise_for_execution_errors` | Function | Assigned parameters into the appropriate place in the input notebook  Parameter… |
| `remove_error_markers` | Function |  |
| `write_ipynb` | Function | Saves a notebook object to the specified path. Args:     nb_node (nbformat.Note… |
| `add_builtin_parameters` | Function | Add built-in parameters to a dictionary of parameters  Parameters ---------- pa… |
| `any_tagged_cell` | Function | Whether the notebook contains at least one cell tagged ``tag``?  Parameters ---… |
| `display_notebook_help` | Function | Display help on notebook parameters.  Parameters ---------- ctx : click.Context… |
| `find_first_tagged_cell_index` | Function | Find the first tagged cell ``tag`` in the notebook.  Parameters ---------- nb :… |
| `get_pretty_path` | Function |  |
| `inspect_notebook` | Function | Return the inferred notebook parameters.  Parameters ---------- notebook_path :… |
| `load_notebook_node` | Function | Returns a notebook object with papermill metadata loaded from the specified pat… |
| `local_file_io_cwd` | Function |  |
...
| `read` | Method |  |
| `write` | Method |  |
| `AzureBlobStore` | Function |  |
| `GCSFileSystem` | Function |  |
| `GCSHandler` | Class |  |
| `listdir` | Method |  |
| `pretty_path` | Method |  |
| `read` | Method |  |

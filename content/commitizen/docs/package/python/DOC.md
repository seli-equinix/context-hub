...
| `generate_ordered_changelog_tree` | Function |  |
| `generate_tree_from_commits` | Function |  |
| `get_changelog_template` | Function |  |
| `get_commit_tag` | Function |  |
| `get_next_tag_name_after_version` | Function |  |
| `get_oldest_and_newest_rev` | Function | Find the tags for the given version.  `version` may come in different formats:… |
| `get_smart_tag_range` | Function | Smart because it finds the N+1 tag.  This is because we need to find until the… |
| `incremental_build` | Function | Takes the original lines and updates with new_content.  The metadata governs ho… |
| `process_commit_message` | Function |  |
| `render_changelog` | Function |  |
| `ChangelogFormat` | Class | Base class for protocol classes.  Protocol classes are defined as::      class… |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_metadata` | Method | Extract the changelog metadata. |
| `ChangelogFormatUnknown` | Class | Unknown `changelog_format` or cannot be determined by the file extension |
| `get_changelog_format` | Function | Get a format from its name  :raises FormatUnknown: if a non-empty name is provi… |
| `AsciiDoc` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
| `get_metadata_from_file` | Method |  |
| `parse_title_level` | Method | Get the title level/type of a line if any |
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `BaseFormat` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
| `get_metadata_from_file` | Method |  |
...
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `BaseConfig` | Class |  |
| `contains_commitizen_section` | Method | Check if the config file contains a commitizen section.  The implementation is… |
| `init_empty_config_content` | Method | Create a config file with the empty config content.  The implementation is diff… |
| `set_key` | Method | Set or update a key in the config file.  Currently, only strings are supported… |
| `update` | Method |  |
| `BaseFormat` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
...
| `normalize_tag` | Method | The tag and the software version might be different.  That's why this function… |
| `search_version` | Method | Search the first or last version tag occurrence in text.  It searches for compl… |
| `VersionTag` | Class | Represent a version and its matching tag form. |
| `get_version_scheme` | Function | Get the version scheme as defined in the configuration or from an overridden `n… |
| `BaseFormat` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
...
| `parse_title_level` | Method | Get the title level/type of a line if any |
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `Markdown` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
| `get_metadata_from_file` | Method |  |
...
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `Metadata` | Class | Metadata extracted from the changelog produced by a plugin |
| `RestructuredText` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
| `get_metadata_from_file` | Method | RestructuredText section titles are not one-line-based, they spread on 2 or 3 l… |
| `parse_title_level` | Method | Get the title level/type of a line if any |
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `BaseFormat` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
...
| `parse_title_level` | Method | Get the title level/type of a line if any |
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `Textile` | Class | Base class to extend to implement a changelog file format. |
| `get_latest_full_release` | Method | Extract metadata for the last non-pre-release. |
| `get_latest_full_release_from_file` | Method |  |
| `get_metadata` | Method | Extract the changelog metadata. |
| `get_metadata_from_file` | Method |  |
| `parse_title_level` | Method | Get the title level/type of a line if any |
| `parse_version_from_title` | Method | Extract the version from a title line if any |
| `CommitizenException` | Class | Common base class for all non-exit exceptions. |
| `ExitCode` | Class | Enum where members are also (and must be) ints |
| `ExpectedExit` | Class | Common base class for all non-exit exceptions. |
| `InvalidCommandArgumentError` | Class | The argument provided to the command is invalid (e.g. `cz check -commit-msg-fil… |
| `NoCommandFoundError` | Class | No command found when running Commitizen cli (e.g., `cz --debug`) |
| `ParseKwargs` | Class | Parse arguments in the for `key=value`.  Quoted strings are automatically unquo… |
| `format_usage` | Method |  |
| `commitizen_excepthook` | Function |  |
| `main` | Function |  |
| `parse_no_raise` | Function | Convert the given string to exit codes.  Receives digits and strings and output… |
| `CharacterSetDecodeError` | Class | The character encoding of the command output could not be determined |
| `Command` | Class | Command(out, err, stdout, stderr, return_code) |
| `run` | Function |  |
| `Bump` | Class | Show prompt for the user to create a guided commit. |
| `Changelog` | Class | Generate a changelog based on the commit history. |
| `Check` | Class | Check if the current commit msg matches the commitizen format. |
| `Commit` | Class | Show prompt for the user to create a guided commit. |
| `manual_edit` | Method |  |
| `Example` | Class | Show an example so people understands the rules. |
| `Info` | Class | Show in depth explanation of your rules. |
| `Init` | Class |  |
| `ListCz` | Class | List currently installed rules. |
| `Schema` | Class | Show structure of the rule. |
| `Version` | Class | Get the version of the installed commitizen or the current project. Precedence:… |
| `Bump` | Class | Show prompt for the user to create a guided commit. |
| `BumpArgs` | Class | dict() -> new empty dictionary dict(mapping) -> new dictionary initialized from… |
| `BumpCommitFailedError` | Class | Commit failed when bumping version |
| `BumpTagFailedError` | Class | Tag failed when bumping version |
| `Changelog` | Class | Generate a changelog based on the commit history. |
| `DryRunExit` | Class | Exit due to passing `--dry-run` option |
| `ExpectedExit` | Class | Common base class for all non-exit exceptions. |
| `InvalidManualVersion` | Class | Manually provided version is invalid |
| `NoCommitsFoundError` | Class | No commits found |
| `NoPatternMapError` | Class | bump / changelog pattern or map can not be found in configuration file |
| `NoneIncrementExit` | Class | The commits found are not eligible to be bumped |
| `NotAGitProjectError` | Class | Not in a git project |
| `NotAllowed` | Class | `--incremental` cannot be combined with a `rev_range` |
| `Settings` | Class | dict() -> new empty dictionary dict(mapping) -> new dictionary initialized from… |
| `TagRules` | Class | Encapsulate tag-related rules.  It allows to filter or match tags according to… |
| `extract_version` | Method | Extract a version from the tag as defined in tag formats.  Raises `InvalidVersi… |
| `find_tag_for` | Method | Find the first matching tag for a given version. |
| `from_settings` | Method | Extract tag rules from settings |
...
| `normalize_tag` | Method | The tag and the software version might be different.  That's why this function… |
| `search_version` | Method | Search the first or last version tag occurrence in text.  It searches for compl… |
| `VersionProtocol` | Class | Base class for protocol classes.  Protocol classes are defined as::      class… |
| `bump` | Method | Based on the given increment, generate the next bumped version according to the… |
| `get_changelog_format` | Function | Get a format from its name  :raises FormatUnknown: if a non-empty name is provi… |
| `get_provider` | Function | Get the version provider as defined in the configuration  :raises VersionProvid… |
| `get_version_scheme` | Function | Get the version scheme as defined in the configuration or from an overridden `n… |
| `Changelog` | Class | Generate a changelog based on the commit history. |
| `ChangelogArgs` | Class | dict() -> new empty dictionary dict(mapping) -> new dictionary initialized from… |
| `DryRunExit` | Class | Exit due to passing `--dry-run` option |
| `GitTag` | Class |  |
| `from_line` | Method |  |
| `NoCommitsFoundError` | Class | No commits found |
| `NoPatternMapError` | Class | bump / changelog pattern or map can not be found in configuration file |
| `NoRevisionError` | Class | No revision found |
| `NotAGitProjectError` | Class | Not in a git project |
| `NotAllowed` | Class | `--incremental` cannot be combined with a `rev_range` |
| `TagRules` | Class | Encapsulate tag-related rules.  It allows to filter or match tags according to… |
| `extract_version` | Method | Extract a version from the tag as defined in tag formats.  Raises `InvalidVersi… |

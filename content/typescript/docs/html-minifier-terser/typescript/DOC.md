...
metadata:
  ...
  source: maintainer
  tags: "typescript,html-minifier-terser,html,minify,build,types,definitelytyped,console,log,build-tool,terser,clean-css"
---
...
## API surface — verifiable exports of `html-minifier-terser`

Each symbol below is a real export of `html-minifier-terser`, verified via `Object.keys(require('html-minifier-terser'))`.

```typescript
```

```javascript
const r_minify = await minify(input);
```

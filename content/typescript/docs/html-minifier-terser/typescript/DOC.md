...
metadata:
  ...
  source: maintainer
  tags: "typescript,html-minifier-terser,html,minify,build,types,definitelytyped,htmlMinifierTerser,console,log,Options,minifyJS,minifyCSS,collapseWhitespace,removeComments,@types/html-minifier-terser,build-tool,terser,clean-css"
---
...
## API surface — html-minifier-terser typed API

```typescript
class Options {}
class CustomEventHandlersOptions {}
class CssOptions {}
class JsOptions {}
class IgnoreCustomCommentsOption {}
class IgnoreCustomFragmentsOption {}
class CustomAttrAssignsOption {}
class CustomAttrCollapseOption {}
class CustomAttrSurroundOption {}
class HtmlMinifierTerser {}
class MinifyResult {}
class MinifierFunction {}
class MinifyError {}
class TerserOptions {}
class CleanCssOptions {}
class HtmlMinifierOptions {}
class CollapseWhitespaceOptions {}
class RemoveAttributeQuotes {}
class RemoveCommentsOptions {}
class CaseSensitiveOptions {}
class ConservativeCollapseOptions {}
class TrimCustomFragments {}
class IncludeAutoGeneratedTags {}
class KeepClosingSlash {}
```

```javascript
const { minify } = require('html-minifier-terser');
const minified = await minify(html, options);
const stripped = await minify(html, { collapseWhitespace: true });
const noComments = await minify(html, { removeComments: true });
const compact = await minify(html, { minifyJS: true, minifyCSS: true });
const conservative = await minify(html, { conservativeCollapse: true });
const stripCdata = await minify(html, { removeCDATASectionsFromCDATA: true });
const preserveLine = await minify(html, { preserveLineBreaks: true });
const result = await minify(html, { useShortDoctype: true });
```
...
class TagOpts {}
class CdataOpts {}
class WhitespaceOpts {}
class CommentMatchOpts {}
class NoScriptOpts {}
class StyleSrcOpts {}
class AsyncMinifyResult {}
class SyncMinifyResult {}

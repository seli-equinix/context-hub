...
| `SecretContext` | Class | SecretContext(current_index, num_total_secrets, secret, header, snippet, error) |
| `SecretNotFoundOnSpecifiedLineError` | Class | Common base class for all non-exit exceptions. |
| `audit_baseline` | Function | :raises: InvalidBaselineError |
| `get_baseline_from_file` | Function | :raises: InvalidBaselineError |
| `get_code_snippet` | Function | :param lines: an iterator of lines in the file :param line_number: line which y… |
| `get_raw_secret_from_file` | Function | We're analyzing the contents straight from the baseline, and therefore, we don'… |
| `get_secret_iterator` | Function | :returns: (index, filename, secret) |
| `BasePlugin` | Class |  |
| `analyze_line` | Method | This examines a line and finds all possible secret values in it. |
| `analyze_string` | Method | Yields all the raw secret values within a supplied string. |
| `format_scan_result` | Method |  |
| `json` | Method |  |
| `verify` | Method |  |
| `InvalidBaselineError` | Class | Think of this as a 400, if getting a baseline had a HTTPError code. |
| `LineGetter` | Class | The problem we try to address with this class is to cache the lines of a transf… |
| `open_file` | Method | This is split up into a different function, so it can be overridden if necessar… |
| `NamedIO` | Class | Base class for text I/O.  This class provides a character and line based interf… |
| `NoLineNumberError` | Class | Common base class for all non-exit exceptions. |
| `PotentialSecret` | Class | This custom data type represents a string found, matching the plugin rules defi… |
| `hash_secret` | Method | This offers a way to coherently test this class, without mocking self.secret_ha… |
| `json` | Method | Custom JSON encoder |
...
| `scan_files` | Method | Just like scan_file, but optimized through parallel processing. |
| `trim` | Method | Removes invalid entries in the current SecretsCollection.  This behaves *kinda*… |
| `call_function_with_arguments` | Function | :raises: TypeError |
| `get_baseline_from_file` | Function | :raises: InvalidBaselineError |
| `get_code_snippet` | Function | :param lines: an iterator of lines in the file :param line_number: line which y… |
| `get_raw_secret_from_file` | Function | We're analyzing the contents straight from the baseline, and therefore, we don'… |
| `get_raw_secrets_from_file` | Function | We're analyzing the contents straight from the baseline, and therefore, we don'… |
| `get_transformed_file` | Function |  |
| `AnsiColor` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `BidirectionalIterator` | Class |  |
| `can_step_back` | Method |  |
| `next` | Method |  |
| `step_back_on_next_iteration` | Method |  |
...
| `get_code_snippet` | Function | :param lines: an iterator of lines in the file :param line_number: line which y… |
| `get_raw_secret_from_file` | Function | We're analyzing the contents straight from the baseline, and therefore, we don'… |
| `transient_settings` | Function | Allows the customizability of non-global settings per invocation. |
| `AnsiColor` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `InputOptions` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `SecretContext` | Class | SecretContext(current_index, num_total_secrets, secret, header, snippet, error) |
| `UserPrompt` | Class |  |
| `clear_screen` | Function |  |
| `colorize` | Function |  |
| `get_user_decision` | Function | :param prompt_secret_decision: if False, won't ask to label secret.     e.g. if… |
| `print_context` | Function |  |
| `print_error` | Function |  |
| `print_message` | Function |  |
| `print_secret_not_found` | Function |  |
| `BidirectionalIterator` | Class |  |
| `can_step_back` | Method |  |
| `next` | Method |  |
| `step_back_on_next_iteration` | Method |  |
...
| `LineGetter` | Class | The problem we try to address with this class is to cache the lines of a transf… |
| `open_file` | Method | This is split up into a different function, so it can be overridden if necessar… |
| `SecretClassToPrint` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `VerifiedResult` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `generate_report` | Function |  |
| `get_baseline_from_file` | Function | :raises: InvalidBaselineError |
| `get_prioritized_verified_result` | Function |  |
| `get_raw_secrets_from_file` | Function | We're analyzing the contents straight from the baseline, and therefore, we don'… |
| `PotentialSecret` | Class | This custom data type represents a string found, matching the plugin rules defi… |
| `hash_secret` | Method | This offers a way to coherently test this class, without mocking self.secret_ha… |
| `json` | Method | Custom JSON encoder |
...
| `scan_files` | Method | Just like scan_file, but optimized through parallel processing. |
| `trim` | Method | Removes invalid entries in the current SecretsCollection.  This behaves *kinda*… |
| `UnableToReadBaselineError` | Class | Think of this as a 404, if getting a baseline had a HTTPError code. |
| `Version` | Class |  |
| `configure_settings_from_baseline` | Function | :raises: KeyError |
| `create` | Function | Scans all the files recursively in path to initialize a baseline. |
| `format_for_output` | Function |  |
| `get_files_to_scan` | Function | If we specify specific files, we should be able to scan them. This abides by th… |
| `import_modules_from_package` | Function |  |
| `load` | Function | With a given baseline file, load all settings and discovered secrets from it.… |
| `load_from_file` | Function | :raises: UnableToReadBaselineError :raises: InvalidBaselineError |
| `save_to_file` | Function | :param secrets: if this is a SecretsCollection, it will output the baseline in… |
| `upgrade` | Function | Baselines will eventually require format changes. This function is responsible… |
| `CustomLogger` | Class | Instances of the Logger class represent a single logging channel. A "logging ch… |
| `addFilter` | Method | Add the specified filter to this handler. |
| `addHandler` | Method | Add the specified handler to this logger. |
| `callHandlers` | Method | Pass a record to all relevant handlers.  Loop through all handlers for this log… |
| `critical` | Method | Log 'msg % args' with severity 'CRITICAL'.  To pass exception information, use… |
| `debug` | Method | Log 'msg % args' with severity 'DEBUG'.  To pass exception information, use the… |
| `error` | Method | Log 'msg % args' with severity 'ERROR'.  To pass exception information, use the… |
| `exception` | Method | Convenience method for logging an ERROR with exception information. |
| `fatal` | Method | Don't use this method, use critical() instead. |
| `filter` | Method | Determine if a record is loggable by consulting all the filters.  The default i… |
| `findCaller` | Method | Find the stack frame of the caller so that we can note the source file name, li… |
| `getChild` | Method | Get a logger which is a descendant to this one.  This is a convenience method,… |
| `getChildren` | Method |  |
| `getEffectiveLevel` | Method | Get the effective level for this logger.  Loop through this logger and its pare… |
| `handle` | Method | Call the handlers for the specified record.  This method is used for unpickled… |
| `hasHandlers` | Method | See if this logger has any handlers configured.  Loop through all handlers for… |
| `info` | Method | Log 'msg % args' with severity 'INFO'.  To pass exception information, use the… |
| `isEnabledFor` | Method | Is this logger enabled for level 'level'? |
| `log` | Method | Log 'msg % args' with the integer severity 'level'.  To pass exception informat… |
| `makeRecord` | Method | A factory method which can be overridden in subclasses to create specialized Lo… |
| `removeFilter` | Method | Remove the specified filter from this handler. |
| `removeHandler` | Method | Remove the specified handler from this logger. |
| `setLevel` | Method | Set the logging level of this logger.  level must be an int or a str. |
| `set_debug_level` | Method | :param debug_level: between 0-2, configure verbosity of log |
| `warn` | Method |  |
| `warning` | Method | Log 'msg % args' with severity 'WARNING'.  To pass exception information, use t… |
| `get_logger` | Function | :param name: used for declaring log channels. :param format_string: for custom… |
| `AnsiColor` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `PotentialSecret` | Class | This custom data type represents a string found, matching the plugin rules defi… |
| `hash_secret` | Method | This offers a way to coherently test this class, without mocking self.secret_ha… |
| `json` | Method | Custom JSON encoder |
...
| `set_secret` | Method |  |
| `colorize` | Function |  |
| `convert_local_os_path` | Function |  |
| `CodeSnippet` | Class |  |
| `add_line_numbers` | Method |  |
| `apply_highlight` | Method | Broken out, for custom colorization. |
| `get_line_number` | Method | Broken out, for custom colorization. |
| `highlight_line` | Method | :param payload: string to highlight, on chosen line |
| `NamedIO` | Class | Base class for text I/O.  This class provides a character and line based interf… |
| `PotentialSecret` | Class | This custom data type represents a string found, matching the plugin rules defi… |
| `hash_secret` | Method | This offers a way to coherently test this class, without mocking self.secret_ha… |
| `json` | Method | Custom JSON encoder |

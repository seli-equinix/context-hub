...
| `register_config` | Method |  |
| `unused` | Method | :returns: Return a list of keys present in the config source but not used |
| `CircularChainError` | Class | circular chain in config |
| `ConfigConstantDefinition` | Class | A configuration definition whose value is defined upfront (such as the tox envi… |
| `ConfigDefinition` | Class | Abstract base class for configuration definitions. |
| `ConfigDynamicDefinition` | Class | A configuration definition that comes from a source (such as in memory, an ini… |
| `ConfigLoadArgs` | Class | Arguments that help loading a configuration value. |
| `copy` | Method | :returns: create a copy of the object |
| `Loader` | Class | Loader loads configuration values and converts it.  :param overrides: A list of… |
| `build` | Method | Materialize the raw configuration value from the loader.  :param future: a futu… |
| `found_keys` | Method | A list of configuration keys found within the configuration. |
| `load` | Method | Load a value (raw and then convert).  :param key: the key under it lives :param… |
| `load_raw` | Method | Load the raw object from the config store.  :param key: the key under what we w… |
| `to` | Method | Convert given raw type to python type.  :param raw: the raw type :param of_type… |
| `to_bool` | Method | Convert to boolean.  :param value: the value to convert  :returns: a boolean re… |
| `to_command` | Method | Convert to a command to execute.  :param value: the value to convert  :returns:… |
| `to_dict` | Method | Convert to dictionary.  :param value: the value to convert :param of_type: a tu… |
| `to_env_list` | Method | Convert to a tox EnvList.  :param value: the value to convert  :returns: a list… |
| `to_list` | Method | Convert to list.  :param value: the value to convert :param of_type: the type o… |
| `to_path` | Method | Convert to path.  :param value: the value to convert  :returns: path representa… |
| `to_set` | Method | Convert to set.  :param value: the value to convert :param of_type: the type of… |
| `to_str` | Method | Convert to string.  :param value: the value to convert  :returns: a string repr… |
| `ConfigLoadArgs` | Class | Arguments that help loading a configuration value. |
| `copy` | Method | :returns: create a copy of the object |
| `Fail` | Class | Failed creating env. |
| `SetEnv` | Class |  |
| `load` | Method |  |
| `update` | Method |  |
| `use_replacer` | Method |  |
...
| `register_config` | Method |  |
| `unused` | Method | :returns: Return a list of keys present in the config source but not used |
| `EnvList` | Class | A tox environment list. |
| `SetEnv` | Class |  |
| `load` | Method |  |
| `update` | Method |  |
| `use_replacer` | Method |  |
| `Source` | Class | Source is able to return a configuration value (for either the core or per envi… |
| `envs` | Method | :param core_conf: the core configuration set  :returns: a list of environments… |
| `get_base_sections` | Method |  |
| `get_core_section` | Method | :returns: the core section |
| `get_loader` | Method |  |
| `get_loaders` | Method | Return a loader that loads settings from a given section name.  :param section:… |
| `get_tox_env_section` | Method | :returns: the section for a tox environment |
| `sections` | Method | Return a loader that loads the core configuration values.  :returns: the core l… |
| `transform_section` | Method |  |
| `discover_source` | Function | Discover a source for configuration.  :param config_file: the file storing the… |
| `CircularChainError` | Class | circular chain in config |
| `Command` | Class | A command to execute. |
| `EnvList` | Class | A tox environment list. |
| `MissingRequiredConfigKeyError` | Class | missing required config key  Used by the two toml loaders in order to identify… |
| `shell_cmd` | Function |  |
| `ExecuteRequest` | Class | Defines a commands execution request. |
| `Outcome` | Class | Result of a command execution. |
| `assert_failure` | Method | Assert that the execution failed. |
| `assert_success` | Method | Assert that the execution succeeded. |
| `log_run_done` | Method | Log that the run was done.  :param lvl: the level on what to log as interpreted… |
| `out_err` | Method | :returns: a tuple of the standard output and standard error |
| `Execute` | Class | Abstract API for execution of a tox environment. |
| `build_instance` | Method |  |
| `call` | Method |  |
| `register_conf` | Method |  |
| `ExecuteInstance` | Class | An instance of a command execution. |
| `ExecuteOptions` | Class |  |
| `register_conf` | Method |  |
| `ExecuteRequest` | Class | Defines a commands execution request. |
| `ExecuteStatus` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `interrupt` | Method |  |
| `set_out_err` | Method |  |
| `wait` | Method |  |
| `write_stdin` | Method |  |
...
| `log_run_done` | Method | Log that the run was done.  :param lvl: the level on what to log as interpreted… |
| `out_err` | Method | :returns: a tuple of the standard output and standard error |
| `StdinSource` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `SyncWrite` | Class | Make sure data collected is synced in-memory and to the target stream on every… |
| `colored` | Method |  |
| `handler` | Method | A callback called whenever content is written. |
| `Execute` | Class | Abstract API for execution of a tox environment. |
| `build_instance` | Method |  |
| `call` | Method |  |
| `register_conf` | Method |  |
...
| `write_stdin` | Method |  |
| `Fail` | Class | Failed creating env. |
| `LocalSubProcessExecuteInstance` | Class | An instance of a command execution. |
| `get_stream_file_no` | Method |  |
| `set_out_err` | Method |  |
| `LocalSubProcessExecutor` | Class | Abstract API for execution of a tox environment. |
| `build_instance` | Method |  |
| `call` | Method |  |
| `register_conf` | Method |  |
| `LocalSubprocessExecuteFailedStatus` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `interrupt` | Method |  |
| `set_out_err` | Method |  |
| `wait` | Method |  |
| `write_stdin` | Method | Cannot write. |
| `LocalSubprocessExecuteStatus` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `interrupt` | Method |  |
| `set_out_err` | Method |  |
| `wait` | Method |  |
| `write_stdin` | Method |  |
| `ReadViaThread` | Class | Helper class that provides a standard way to create an ABC using inheritance. |
| `StdinSource` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `shebang` | Function | :param exe: the executable  :returns: the shebang interpreter arguments |
| `Execute` | Class | Abstract API for execution of a tox environment. |
| `build_instance` | Method |  |
| `call` | Method |  |
| `register_conf` | Method |  |
...
| `get_stream_file_no` | Method |  |
| `set_out_err` | Method |  |
| `LocalSubProcessPep517ExecuteInstance` | Class | A backend invocation. |
| `LocalSubProcessPep517Executor` | Class | Executor holding the backend process. |
| `build_instance` | Method |  |
| `call` | Method |  |
| `close` | Method |  |
| `local_execute` | Method |  |
| `register_conf` | Method |  |
| `StdinSource` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |

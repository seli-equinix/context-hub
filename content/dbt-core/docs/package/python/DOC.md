...
| `add_catalog_integration` | Method |  |
| `already_exists` | Method | DEPRECATED: Return if a model already exists in the database |
| `assert_valid_snapshot_target_given_strategy` | Method |  |
| `build_catalog_relation` | Method |  |
| `builtin_incremental_strategies` | Method | List of possible builtin strategies for adapters  Microbatch is added by _defau… |
| `cache_added` | Method | Cache a new relation in dbt. It will show up in `list relations`. |
| `cache_dropped` | Method | Drop a relation in dbt. It will no longer show up in `list relations`, and any… |
| `cache_renamed` | Method | Rename a relation in dbt. It will show up with a new name in `list_relations`,… |
| `calculate_freshness` | Method | Calculate the freshness of sources in dbt, and return it |
| `calculate_freshness_from_custom_sql` | Method |  |
| `calculate_freshness_from_metadata` | Method |  |
| `calculate_freshness_from_metadata_batch` | Method | Given a list of sources (BaseRelations), calculate the metadata-based freshness… |
| `cancel_open_connections` | Method | Cancel all open connections. |
| `capabilities` | Method |  |
| `check_schema_exists` | Method | Check if a schema exists.  The default implementation of this is potentially un… |
| `cleanup_connections` | Method |  |
| `clear_macro_resolver` | Method |  |
| `clear_transaction` | Method |  |
| `commit_if_has_connection` | Method |  |
| `connection_named` | Method |  |
| `convert_agate_type` | Method |  |
| `convert_boolean_type` | Method | Return the type in the database that best maps to the agate.Boolean type for th… |
| `convert_date_type` | Method | Return the type in the database that best maps to the agate.Date type for the g… |
| `convert_datetime_type` | Method | Return the type in the database that best maps to the agate.DateTime type for t… |
| `convert_integer_type` | Method | Return the type in the database that best maps to the agate.Number type for the… |
| `convert_number_type` | Method | Return the type in the database that best maps to the agate.Number type for the… |
| `convert_text_type` | Method | Return the type in the database that best maps to the agate.Text type for the g… |
| `convert_time_type` | Method | Return the type in the database that best maps to the agate.TimeDelta type for… |
| `convert_type` | Method |  |
| `BaseConnectionManager` | Class | Methods to implement:     - exception_handler     - cancel_open     - open… |
| `add_select_query` | Method | This was added here because base.impl.BaseAdapter.get_column_schema_from_query… |
| `begin` | Method | Begin a transaction. (passable) |
| `cancel_open` | Method | Cancel all open connections on the adapter. (passable) |
| `cleanup_all` | Method |  |
| `clear_thread_connection` | Method |  |
| `clear_transaction` | Method | Clear any existing transactions. |
| `close` | Method |  |
| `commit` | Method | Commit a transaction. (passable) |
| `commit_if_has_connection` | Method | If the named connection exists, commit the current transaction. |
| `data_type_code_to_name` | Method | Get the string representation of the data type from the type_code. |
| `exception_handler` | Method | Create a context manager that handles exceptions caused by database interaction… |
| `execute` | Method | Execute the given SQL.  :param str sql: The sql to execute. :param bool auto_be… |
| `get_if_exists` | Method |  |
| `get_thread_connection` | Method |  |
| `get_thread_identifier` | Method |  |
| `open` | Method | Open the given connection on the adapter and return it.  This may mutate the gi… |
| `release` | Method |  |
| `retry_connection` | Method | Given a Connection, set its handle by calling connect.  The calls to connect wi… |
| `rollback_if_open` | Method |  |
| `set_connection_name` | Method | Called by 'acquire_connection' in BaseAdapter, which is called by 'connection_n… |
| `set_query_header` | Method |  |
| `set_thread_connection` | Method |  |
| `BaseRelation` | Class | BaseRelation(path: dbt.adapters.contracts.relation.Path, type: Optional[dbt.ada… |
| `add_ephemeral_prefix` | Method |  |
| `create` | Method |  |
| `create_ephemeral_from` | Method |  |
| `create_from` | Method |  |
| `from_dict` | Method |  |
| `from_msgpack` | Method |  |
| `get` | Method | Override `.get` to return a metadata object so we don't break dbt_utils. |
| `get_default_include_policy` | Method |  |
| `get_default_quote_policy` | Method |  |
| `get_function_config` | Method |  |
| `get_function_macro_name` | Method |  |
| `include` | Method |  |
| `incorporate` | Method |  |
| `information_schema` | Method |  |
| `information_schema_only` | Method |  |
| `items` | Method | D.items() -> a set-like object providing a view on D's items |
| `json_schema` | Method |  |
| `keys` | Method | D.keys() -> a set-like object providing a view on D's keys |
| `matches` | Method |  |
...
| `quoted` | Method |  |
| `render` | Method |  |
| `render_event_time_filtered` | Method |  |
| `render_limited` | Method |  |
| `replace` | Method |  |
| `replace_path` | Method |  |
| `scd_args` | Method |  |
| `to_dict` | Method |  |
| `to_msgpack` | Method |  |
| `validate` | Method |  |
| `Column` | Class | Column(column: str, dtype: str, char_size: Optional[int] = None, numeric_precis… |
| `can_expand_to` | Method | returns True if this column can be expanded to the size of the other column |
| `create` | Method |  |
| `from_description` | Method |  |
| `is_float` | Method |  |
| `is_integer` | Method |  |
| `is_number` | Method |  |
| `is_numeric` | Method |  |
...
| `string_size` | Method |  |
| `string_type` | Method |  |
| `translate_type` | Method |  |
| `ConstraintSupport` | Class | str(object='') -> str str(bytes_or_buffer[, encoding[, errors]]) -> str  Create… |
| `PythonJobHelper` | Class |  |
| `submit` | Method |  |
| `PythonSubmissionResult` | Class | Result from submitting a Python job. |
| `RelationType` | Class | str(object='') -> str str(bytes_or_buffer[, encoding[, errors]]) -> str  Create… |
| `SchemaSearchMap` | Class | A utility class to keep track of what information_schema tables to search for w… |
| `add` | Method |  |
| `flatten` | Method |  |
| `search` | Method |  |
| `CacheAction` | Class | BaseEvent for proto message generated python events. |
| `code` | Method |  |
| `format_ref_key` | Method |  |
| `level_tag` | Method |  |
| `message` | Method |  |
| `to_dict` | Method |  |
| `to_json` | Method |  |
| `CacheDumpGraph` | Class | BaseEvent for proto message generated python events. |
| `code` | Method |  |
| `level_tag` | Method |  |
| `message` | Method |  |
| `to_dict` | Method |  |
| `to_json` | Method |  |
| `DependentLinkNotCachedError` | Class | Common base class for all non-exit exceptions. |
| `data` | Method |  |
| `process_stack` | Method |  |
| `NewNameAlreadyInCacheError` | Class | Common base class for all non-exit exceptions. |
| `data` | Method |  |
| `process_stack` | Method |  |
| `NoneRelationFoundError` | Class | Common base class for all non-exit exceptions. |
| `data` | Method |  |
| `process_stack` | Method |  |
| `ReferencedLinkNotCachedError` | Class | Common base class for all non-exit exceptions. |
| `data` | Method |  |
| `process_stack` | Method |  |
| `RelationsCache` | Class | A cache of the relations known to dbt. Keeps track of relationships declared be… |
| `add` | Method | Add the relation inner to the cache, under the schema schema and identifier ide… |
| `add_link` | Method | Add a link between two relations to the database. If either relation does not e… |
| `add_schema` | Method | Add a schema to the set of known schemas (case-insensitive)  :param database: T… |
| `clear` | Method | Clear the cache |
| `drop` | Method | Drop the named relation and cascade it appropriately to all dependent relations… |
| `drop_schema` | Method | Drop the given schema and remove it from the set of known schemas.  Then remove… |
| `dump_graph` | Method | Dump a key-only representation of the schema to a dictionary. Every known relat… |
| `get_relations` | Method | Case-insensitively yield all relations matching the given schema.  :param str s… |
| `rename` | Method | Rename the old schema/identifier to the new schema/identifier and update refere… |
| `update_schemas` | Method | Add multiple schemas to the set of known schemas (case-insensitive)  :param sch… |
| `TruncatedModelNameCausedCollisionError` | Class | Common base class for all non-exit exceptions. |
| `data` | Method |  |
| `get_message` | Method |  |
| `process_stack` | Method |  |
| `dot_separated` | Function | Return the key in dot-separated string form.  :param _ReferenceKey key: The key… |
| `fire_event` | Function |  |
| `fire_event_if` | Function |  |
| `lowercase` | Function |  |
| `Capability` | Class | Enumeration of optional adapter features which can be probed using BaseAdapter.… |
| `CapabilityDict` | Class | defaultdict(default_factory=None, /, [...]) --> dict with default factory  The… |
| `CapabilitySupport` | Class | CapabilitySupport(support: dbt.adapters.capability.Support, first_version: Opti… |
| `Support` | Class | str(object='') -> str str(bytes_or_buffer[, encoding[, errors]]) -> str  Create… |
| `CatalogIntegration` | Class | Represent a catalog integration for a given user config  This class should be i… |
| `build_relation` | Method | Builds relation configuration within the context of this catalog integration.… |
| `CatalogIntegrationClient` | Class | A repository class that manages catalog integrations  This class manages all ty… |
| `add` | Method |  |
| `get` | Method |  |
| `CatalogIntegrationConfig` | Class | Represents the user configuration required to describe a catalog integration  T… |
| `CatalogRelation` | Class | Base class for protocol classes.  Protocol classes are defined as::      class… |
| `DbtCatalogIntegrationAlreadyExistsError` | Class | Unspecified run-time error. |
| `add_node` | Method |  |
| `data` | Method |  |
| `node_to_string` | Method | Given a node-like object we attempt to create the best identifier we can. |
| `process_stack` | Method |  |
| `validator_error_message` | Method | Given a dbt.dataclass_schema.ValidationError return the relevant parts as a str… |
| `DbtCatalogIntegrationNotFoundError` | Class | Unspecified run-time error. |
| `add_node` | Method |  |
| `data` | Method |  |

...
| `get` | Method | Return the value for key if key is in the dictionary, else default. |
| `path` | Method |  |
| `FilterDefaults` | Class | Override `FilterDefaults` to change which notebook processors are used |
| `base_procs` | Method |  |
| `nb_proc` | Method | Get an `NBProcessor` with these processors |
| `procs` | Method | Processors for export |
| `xtra_procs` | Method |  |
| `FrontmatterProc` | Class | A YAML and formatted-markdown frontmatter processor |
| `begin` | Method |  |
| `cell` | Method |  |
| `end` | Method |  |
| `NBProcessor` | Class | Process cells and nbdev comments in a notebook |
| `process` | Method | Process all cells with all processors |
| `NbdevLookup` | Class | Mapping from symbol names to docs and source URLs |
| `code` | Method | Link to source code for `sym` |
| `doc` | Method | Link to docs for `sym` |
| `link_line` | Method |  |
| `linkify` | Method |  |
| `Processor` | Class | Base class for processors |
| `cell` | Method |  |
| `add_fold` | Function | Add `code-fold` to `exports` cells |
| `add_init` | Function | Add `__init__.py` in all subdirs of `path` containing python files if it's not… |
| `add_links` | Function | Add links to markdown cells |
| `add_show_docs` | Class | Add show_doc cells after exported cells, unless they are already documented |
| `begin` | Method |  |
| `cell` | Method |  |
| `ai_magics` | Function | A preprocessor to convert AI magics to markdown |
| `boxify` | Function | Add a box around `cells` |
| `bump_version` | Function | Bump semver string `v` at index `part` (0=major, 1=minor, 2=patch) |
| `cell_lang` | Function |  |
| `chelp` | Function | Show help for all console scripts |
| `clean_jupyter` | Function | Clean Jupyter `model` pre save to `path` |
| `clean_magics` | Function | A preprocessor to remove cell magic commands |
| `clean_nb` | Function | Clean `nb` from superfluous metadata |
| `clean_show_doc` | Function | Remove ShowDoc input cells |
| `create_index` | Function | Create a documentation index from a sphinx inventory file at `url`, with option… |
| `create_output` | Function | Add a cell output containing `txt` of the `mime` text MIME sub-type |
| `exec_show_docs` | Class | Execute cells needed for `show_docs` output, including exported cells and impor… |
| `begin` | Method |  |
| `cell` | Method |  |
| `end` | Method |  |
| `extract_directives` | Function | Take leading comment directives from lines of code in `ss`, remove `#\|`, and sp… |
| `extract_tgz` | Function |  |
| `fdiv` | Function | Create a fenced div markdown cell in quarto |
| `filter_stream_` | Function | Remove output lines containing any of `words` in `cell` stream output |
| `first_code_ln` | Function | get first line number where code occurs, where `code_list` is a list of code |
| `fs_watchdog` | Function | File system watchdog dispatching to `func` |
| `get_config` | Function | Return nbdev config. |
| `hide_` | Function | Hide cell from output |
| `hide_line` | Function | Hide lines of code in code cells with the directive `hide_line` at the end of a… |
| `import_obj` | Function | Import and return `module:obj` string |
| `insert_warning` | Class | Insert Autogenerated Warning Into Notebook after the first cell. |
| `begin` | Method |  |
| `cell` | Method |  |
| `is_nbdev` | Function |  |
| `mv_exports` | Class | Move `exports` cells to after the `show_doc` |
| `begin` | Method |  |
| `cell` | Method |  |
| `nb_export` | Function | Create module(s) from notebook |
| `nb_export_cli` | Function | Export a single nbdev notebook to a python script. |
| `nb_lang` | Function |  |
| `nbdev_clean` | Function | Clean all notebooks in `fname` to avoid merge conflicts |
| `nbdev_contributing` | Function | Create CONTRIBUTING.md from contributing_nb (defaults to 'contributing.ipynb' i… |
| `nbdev_create_config` | Function | Create a pyproject.toml config file. |
| `nbdev_export` | Function | Export notebooks in `path` to Python modules |
| `nbdev_filter` | Function | A notebook filter for Quarto |
| `nbdev_install_hooks` | Function | Install Jupyter and git hooks to automatically clean, trust, and fix merge conf… |
| `nbdev_new` | Function | Create an nbdev project. |
| `nbdev_readme` | Function | Create README.md from readme_nb (index.ipynb by default) |
| `nbdev_test` | Function | Test in parallel notebooks matching `path`, passing along `flags` |
| `nbdev_trust` | Function | Trust notebooks matching `fname`. |
| `nbdev_update_license` | Function | Allows you to update the license of your project. |
| `nbglob` | Function | Find all files in a directory matching an extension given a config key. |
| `nbglob_cli` | Function | Find all files in a directory matching an extension given a config key. |
| `opt_set` | Function | newval if newval else var |
| `patch_name` | Function | If `o` is decorated with `patch` or `patch_to`, return its class-prefix name |
| `populate_language` | Class | Set cell language based on NB metadata and magics |
| `begin` | Method |  |
| `cell` | Method |  |
| `process_write` | Function |  |
| `read_version` | Function | Read __version__ from `path/__init__.py`, or None if not found |
| `refresh_quarto_yml` | Function | Generate `_quarto.yml` from `pyproject.toml`. |
| `rm_export` | Function | Remove cells that are exported or hidden |
| `rm_header_dash` | Function | Remove headings that end with a dash - |
| `set_version` | Function | Set __version__ in `path/__init__.py` |
| `show_src` | Function |  |
| `strip_ansi` | Function | Strip Ansi Characters. |
| `strip_hidden_metadata` | Function | Strips "hidden" metadata property from code cells so it doesn't interfere with… |
| `test_nb` | Function | Execute tests in notebook in `fn` except those with `skip_flags` |
| `update_proj` | Function | Create or update `pyproject.toml` in the project root. |
| `update_version` | Function | Add __version__ to `path/__init__.py` if it doesn't exist |
| `watch_export` | Function | Use `nb_export` on ipynb files in `nbs` directory on changes using nbdev config… |
| `write_cells` | Function | Write `cells` to `file` along with header `hdr` (mainly for nbdev internal use). |
| `ConfigToml` | Class | `dict` subclass that also provides access to keys as attrs, and has a pretty ma… |
| `copy` | Method | Return a shallow copy of the dict. |
| `get` | Method | Return the value for key if key is in the dictionary, else default. |
...
| `update_version` | Function | Add __version__ to `path/__init__.py` if it doesn't exist |
| `write_cells` | Function | Write `cells` to `file` along with header `hdr` (mainly for nbdev internal use). |
| `cell_diffs` | Function | {cell_id:diff} for changed/added/deleted cells between two refs |
| `changed_cells` | Function | Return set of cell IDs for changed/added/deleted cells between two refs |
| `nbs_pair` | Function | NBs at two refs; None means working dir. By default provides HEAD and working d… |
| `read_nb_from_git` | Function | Read notebook from git ref (e.g. HEAD) at path, or working dir if ref is None |
| `source_diff` | Function | Return unified diff string for source change |
| `ConfigToml` | Class | `dict` subclass that also provides access to keys as attrs, and has a pretty ma… |
| `copy` | Method | Return a shallow copy of the dict. |
| `get` | Method | Return the value for key if key is in the dictionary, else default. |
| `path` | Method |  |
| `ExportModuleProc` | Class | A processor which exports code to a module |
| `begin` | Method |  |
| `ModuleMaker` | Class | Helper class to create exported library from notebook source cells |
| `make` | Method | Write module containing `cells` with `__all__` generated from `all_cells` |
| `make_all` | Method | Create `__all__` with all exports in `cells` |
| `NbdevLookup` | Class | Mapping from symbol names to docs and source URLs |
| `code` | Method | Link to source code for `sym` |
| `doc` | Method | Link to docs for `sym` |
| `link_line` | Method |  |
...
| `create_index` | Function | Create a documentation index from a sphinx inventory file at `url`, with option… |
| `create_output` | Function | Add a cell output containing `txt` of the `mime` text MIME sub-type |
| `decor_id` | Function | `id` attr of decorator, regardless of whether called as function or bare |
| `find_var` | Function | Find the line numbers where `varname` is defined in `lines` |
| `get_config` | Function | Return nbdev config. |
| `import_obj` | Function | Import and return `module:obj` string |
| `is_nbdev` | Function |  |
| `make_code_cells` | Function |  |
| `nb_export` | Function | Create module(s) from notebook |
| `nbdev_create_config` | Function | Create a pyproject.toml config file. |
| `nbdev_export` | Function | Export notebooks in `path` to Python modules |
| `nbglob` | Function | Find all files in a directory matching an extension given a config key. |
| `nbglob_cli` | Function | Find all files in a directory matching an extension given a config key. |
| `patch_name` | Function | If `o` is decorated with `patch` or `patch_to`, return its class-prefix name |
| `read_var` | Function | Eval and return the value of `varname` defined in `code` |
| `read_version` | Function | Read __version__ from `path/__init__.py`, or None if not found |
| `relative_import` | Function | Convert a module `name` to a name relative to `fname` |
| `set_version` | Function | Set __version__ in `path/__init__.py` |
| `show_src` | Function |  |
| `update_import` | Function |  |
| `update_proj` | Function | Create or update `pyproject.toml` in the project root. |
| `update_var` | Function | Update the definition of `varname` in file `fn`, by calling `func` with the cur… |
| `update_version` | Function | Add __version__ to `path/__init__.py` if it doesn't exist |
| `write_cells` | Function | Write `cells` to `file` along with header `hdr` (mainly for nbdev internal use). |
| `ConfigToml` | Class | `dict` subclass that also provides access to keys as attrs, and has a pretty ma… |
| `copy` | Method | Return a shallow copy of the dict. |

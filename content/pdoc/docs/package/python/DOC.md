...
| `walk_packages2` | Function | For a given list of modules, recursively yield their names and all their submod… |
| `walk_specs` | Function | This function processes a list of module specifications and returns a collectio… |
| `DefaultMacroExtension` | Class | This extension provides a new `{% defaultmacro %}` statement, which defines a m… |
...
| `configure` | Function | Configure the rendering output.  - `docformat` is the docstring flavor in use.… |
| `defuse_unsafe_reprs` | Function | This decorator is applied by pdoc before calling an object's repr(). It applies… |
| `edit_url` | Function | Create a link to edit a particular file in the used version control system. |
| `format_signature` | Function | Format and highlight a function signature using pygments. Returns HTML. |
| `html_error` | Function | Renders an error message. |
| `html_index` | Function | Renders the module index. |
| `html_module` | Function | Renders the documentation for a `pdoc.doc.Module`.  - `all_modules` contains al… |
| `link` | Function | Create a link for a specific `(modulename, qualname)` tuple. |
| `linkify` | Function | Link all identifiers in a block of text. Identifiers referencing unknown module… |
| `make_index` | Function | This method compiles all currently documented modules into a pile of documentat… |
| `minify_css` | Function | Do some very basic CSS minification. |
| `precompile_index` | Function | This method tries to precompile the Elasticlunr.js search index by invoking `no… |
| `repr_module` | Function | Renders `repr(pdoc.doc.Module)`, primarily used for tests and debugging. |
| `root_module_name` | Function | Return the name of the (unique) top-level module, or `None` if no such module e… |
| `search_index` | Function | Renders the Elasticlunr.js search index. |
| `to_markdown_with_context` | Function | Converts `docstring` from a custom docformat to Markdown (if necessary), and th… |
| `DefaultMacroExtension` | Class | This extension provides a new `{% defaultmacro %}` statement, which defines a m… |
| `attr` | Method | Return an attribute node for the current extension.  This is useful to pass con… |
| `bind` | Method | Create a copy of this extension bound to another environment. |
| `call_method` | Method | Call a method of the extension.  This is a shortcut for :meth:`attr` + :class:`… |
...
| `linkify` | Function | Link all identifiers in a block of text. Identifiers referencing unknown module… |
| `minify_css` | Function | Do some very basic CSS minification. |
| `module_candidates` | Function | Given an identifier and the current module name, return the module names we sho… |
| `possible_sources` | Function | For a given identifier, return all possible sources where it could originate fr… |
| `qualname_candidates` | Function | Given an identifier in a current namespace, return all possible qualnames in th… |
| `root_module_name` | Function | Return the name of the (unique) top-level module, or `None` if no such module e… |
| `split_identifier` | Function | Split an identifier into a `(modulename, qualname)` tuple. For example, `pdoc.r… |
...
| `precompile_index` | Function | This method tries to precompile the Elasticlunr.js search index by invoking `no… |
| `to_markdown` | Function |  |
| `AllModules` | Class | A lazy-loading implementation of all_modules.  This behaves like a regular dict… |
...
| `DocHandler` | Class | A handler for individual requests. |
| `address_string` | Method | Return the client address. |
| `date_time_string` | Method | Return the current date and time formatted for a message header. |
| `do_GET` | Method |  |
| `do_HEAD` | Method |  |
| `end_headers` | Method | Send the blank line ending the MIME headers. |
| `finish` | Method |  |
| `flush_headers` | Method |  |
| `handle` | Method | Handle multiple requests if necessary. |
| `handle_expect_100` | Method | Decide what to do with an "Expect: 100-continue" header.  If the client is expe… |
| `handle_one_request` | Method | Handle a single HTTP request.  You normally don't need to override this method;… |
| `handle_request` | Method | Actually handle a request. Called by `do_HEAD` and `do_GET`. |
| `log_date_time_string` | Method | Return the current time formatted for logging. |
| `log_error` | Method | Log an error.  This is called when a request cannot be fulfilled.  By default i… |
| `log_message` | Method | Log an arbitrary message.  This is used by all other logging functions.  Overri… |
| `log_request` | Method | Override logging to disable it. |
| `parse_request` | Method | Parse a request (internal).  The request should be stored in self.raw_requestli… |
| `send_error` | Method | Send and log an error reply.  Arguments are * code:    an HTTP error code… |
| `send_header` | Method | Send a MIME header to the headers buffer. |
| `send_response` | Method | Add the response header to the headers buffer and log the response code.  Also… |
| `send_response_only` | Method | Send the response header only. |
| `setup` | Method |  |
| `version_string` | Method | Return the server software version string. |
| `DocServer` | Class | pdoc's live-reloading web server |
...
| `process_request` | Method | Call finish_request.  Overridden by ForkingMixIn and ThreadingMixIn. |
...
| `server_close` | Method | Called to clean-up the server.  May be overridden. |
...
| `open_browser` | Function | Open a URL in a browser window. In contrast to `webbrowser.open`, we limit the… |

...
| `err` | Function |  |
| `find_pyproject_toml` | Function | Find the absolute filepath to a pyproject.toml if it exists |
| `format_cell` | Function |  |
| `format_file_contents` | Function |  |
| `format_file_in_place` | Function |  |
| `format_ipynb_string` | Function |  |
| `format_stdin_to_stdout` | Function |  |
| `format_str` | Function |  |
| `gen_python_files` | Function | Generate all files under `path` whose paths are not excluded by the `exclude_re… |
| `get_features_used` | Function |  |
| `get_future_imports` | Function |  |
| `get_sources` | Function |  |
| `ipynb_diff` | Function | Return a unified diff string between each cell in notebooks `a` and `b`. |
| `is_number_token` | Function |  |
| `is_simple_decorator_expression` | Function |  |
| `lib2to3_parse` | Function |  |
| `mask_cell` | Function |  |
| `normalize_fmt_off` | Function |  |
| `out` | Function |  |
| `parse_ast` | Function |  |
| `parse_line_ranges` | Function |  |
| `parse_pyproject_toml` | Function | Parse a pyproject toml file, pulling out relevant parts for Black.  If parsing… |
| `patched_main` | Function |  |
| `path_is_excluded` | Function |  |
| `put_trailing_semicolon_back` | Function |  |
| `re_compile_maybe_verbose` | Function |  |
| `read_pyproject_toml` | Function |  |
| `reformat_code` | Function |  |
| `reformat_one` | Function |  |
| `remove_trailing_semicolon` | Function |  |
| `resolves_outside_root_or_cannot_stat` | Function | Returns whether the path is a symbolic link that points outside the root direct… |
| `sanitized_lines` | Function |  |
| `spellcheck_pyproject_toml_keys` | Function |  |
| `stringify_ast` | Function |  |
| `supports_feature` | Function |  |
| `target_version_option_callback` | Function |  |
| `transform_line` | Function |  |
| `unmask_cell` | Function |  |
| `validate_cell` | Function |  |
| `validate_metadata` | Function |  |
| `validate_regex` | Function |  |
| `wrap_stream_for_windows` | Function | Wrap stream with colorama's wrap_stream so colors are shown on Windows.  If `co… |
| `BracketMatchError` | Class |  |
| `BracketTracker` | Class |  |
| `get_leaves_inside_matching_brackets` | Function |  |
| `is_split_after_delimiter` | Function |  |
| `is_split_before_delimiter` | Function |  |
| `is_vararg` | Function |  |
| `max_delimiter_priority_in_atom` | Function |  |
| `Cache` | Class |  |
| `FileData` | Class | mypyc filler docstring |
| `Mode` | Class |  |
| `err` | Function |  |
...
| `Mode` | Class |  |
| `ProtoComment` | Class |  |
| `children_contains_fmt_on` | Function |  |
| `container_of` | Function |  |
| `contains_fmt_directive` | Function |  |
| `contains_pragma_comment` | Function |  |
| `convert_one_fmt_off_pair` | Function |  |
| `first_leaf_of` | Function |  |
| `generate_comments` | Function |  |
| `generate_ignored_nodes` | Function |  |
| `is_fmt_on` | Function |  |
| `is_type_comment_string` | Function |  |
| `make_comment` | Function |  |
| `make_simple_prefix` | Function |  |
| `normalize_fmt_off` | Function |  |
| `normalize_trailing_prefix` | Function |  |
| `preceding_leaf` | Function |  |
| `Cache` | Class |  |
| `Changed` | Class | Create a collection of name/value pairs.  Example enumeration:  >>> class Color… |
| `Mode` | Class |  |
| `Report` | Class | Provides a reformatting counter. Can be rendered with `str(report)`. |
...
| `path_ignored` | Method |  |
| `WriteBack` | Class | mypyc filler docstring |
| `cancel` | Function | asyncio signal handler that cancels all `tasks` and reports to stderr. |
| `err` | Function |  |
| `format_file_in_place` | Function |  |
| `maybe_use_uvloop` | Function | If our environment has uvloop or winloop installed we use it otherwise a normal… |
| `reformat_many` | Function | Reformat multiple files using a ProcessPoolExecutor. |
| `schedule_formatting` | Function | Run formatting of `sources` in parallel using the provided `executor`.  (Use Pr… |
| `shutdown` | Function | Cancel all pending tasks on `loop`, wait for them, and close the loop. |
| `DebugVisitor` | Class | DebugVisitor(tree_depth: int = 0, list_output: list[str] = <factory>, print_out… |
| `out` | Method |  |
| `show` | Method | Pretty-print the lib2to3 AST of a given string of `code`.  Convenience method f… |
| `visit_default` | Method |  |
| `Visitor` | Class |  |
| `lib2to3_parse` | Function |  |
| `out` | Function |  |
| `Report` | Class | Provides a reformatting counter. Can be rendered with `str(report)`. |
...
| `find_pyproject_toml` | Function | Find the absolute filepath to a pyproject.toml if it exists |
| `gen_python_files` | Function | Generate all files under `path` whose paths are not excluded by the `exclude_re… |
| `infer_target_version` | Function | Infer Black's target version from the project metadata in pyproject.toml.  Supp… |
| `parse_pyproject_toml` | Function | Parse a pyproject toml file, pulling out relevant parts for Black.  If parsing… |
| `parse_req_python_specifier` | Function | Parse a specifier string (i.e. ``">=3.7,<3.10"``) to a list of TargetVersion.… |
| `parse_req_python_version` | Function | Parse a version string (i.e. ``"3.7"``) to a list of TargetVersion.  If parsing… |
| `path_is_excluded` | Function |  |
| `resolves_outside_root_or_cannot_stat` | Function | Returns whether the path is a symbolic link that points outside the root direct… |
| `strip_specifier_set` | Function | Strip minor versions for some specifiers in the specifier set.  For background… |
| `wrap_stream_for_windows` | Function | Wrap stream with colorama's wrap_stream so colors are shown on Windows.  If `co… |
| `CellMagic` | Class |  |
| `CellMagicFinder` | Class |  |
| `generic_visit` | Method | Called if no explicit visitor function exists for a node. |
| `visit` | Method | Visit a node. |
| `visit_Constant` | Method |  |
| `MagicFinder` | Class |  |
| `generic_visit` | Method | Called if no explicit visitor function exists for a node. |
| `visit` | Method | Visit a node. |
| `visit_Constant` | Method |  |
...
| `put_trailing_semicolon_back` | Function |  |
| `remove_trailing_semicolon` | Function |  |
| `replace_cell_magics` | Function |  |
| `replace_magics` | Function |  |
| `unmask_cell` | Function |  |
| `validate_cell` | Function |  |
| `CannotSplit` | Class |  |
| `CannotTransform` | Class |  |
| `Feature` | Class | mypyc filler docstring |
| `Line` | Class |  |
| `LineGenerator` | Class |  |
| `Mode` | Class |  |
...
| `RHSResult` | Class |  |
| `StringMerger` | Class |  |
| `StringParenStripper` | Class |  |
| `StringParenWrapper` | Class |  |
| `StringSplitter` | Class |  |
| `Visitor` | Class |  |
| `append_leaves` | Function |  |
| `bracket_split_build_line` | Function |  |
| `bracket_split_succeeded_or_raise` | Function |  |
| `can_be_split` | Function |  |
| `can_omit_invisible_parens` | Function |  |
| `contains_fmt_directive` | Function |  |
| `dont_increase_indentation` | Function |  |
| `ensure_visible` | Function |  |
| `fix_multiline_docstring` | Function |  |
| `fstring_tstring_to_string` | Function |  |
| `generate_comments` | Function |  |
| `generate_trailers_to_omit` | Function |  |
| `get_annotation_type` | Function |  |
| `get_leaves_inside_matching_brackets` | Function |  |
| `get_string_prefix` | Function |  |
| `has_sibling_with_type` | Function |  |
| `hug_power_op` | Function |  |
| `is_arith_like` | Function |  |
| `is_async_stmt_or_funcdef` | Function |  |
| `is_atom_with_invisible_parens` | Function |  |
| `is_docstring` | Function |  |
| `is_empty_tuple` | Function |  |
| `is_generator` | Function |  |

...
Like every ESLint plugin/config, `eslint-plugin-security` integrates with ESLint's public API. The types and helpers below are the stable plugin/config author surface.
...
const messages = linter.verify(code, { plugins: ['eslint-plugin-security'] });

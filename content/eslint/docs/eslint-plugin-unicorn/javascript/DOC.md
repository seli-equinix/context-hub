...
metadata:
  ...
  source: maintainer
  tags: "eslint,unicorn,javascript,linting,node,npm,default,63.0.0,Version-Sensitive,linter,static-analysis,RuleTester,SourceCode,AST,RuleContext,RuleListener,RuleModule,loadESLint,verify,verifyAndFix,lintFiles,lintText,calculateConfigForFile"
---
...
Like every ESLint plugin/config, `eslint-plugin-unicorn` integrates with ESLint's public API. The types and helpers below are the stable plugin/config author surface.
...
const messages = linter.verify(code, { plugins: ['eslint-plugin-unicorn'] });

...
metadata:
  ...
  source: maintainer
  tags: "nuxt,vue,ssr,ssg,nitro,meta-framework,fetch,click,create,latest,composables,useFetch,useAsyncData,useState,useCookie,useRoute,useRouter,useRuntimeConfig,useHead,navigateTo,defineNuxtConfig,defineNuxtPlugin,defineEventHandler,defineNuxtRouteMiddleware,h3"
---
...
## API surface — Nuxt composables, helpers, and config types
...
class NuxtPlugin {}
class NuxtRouter {}
class RouteLocationRaw {}
class RuntimeConfig {}
class AppConfig {}
class FetchOptions {}
class UseFetchOptions {}
class AsyncDataOptions {}
class AsyncData {}
class CookieOptions {}
class HeadOptions {}
class MetaObject {}
...
const item = await useAsyncData('item', () => $fetch('/api/item'));
...
const seo = useSeoMeta({ title: 'Home' });
const error = useError();
const nuxt = useNuxtApp();

// Helpers
const navigated = await navigateTo('/dashboard');
const aborted = abortNavigation('Forbidden');
const refreshed = await refreshNuxtData();
const cleared = await clearNuxtData();
const definitions = defineNuxtConfig({ modules: ['@pinia/nuxt'] });
...
```typescript
class DefineNuxtConfig {}
class DefineNuxtPlugin {}
class DefineNuxtRouteMiddleware {}
class DefineEventHandler {}
class DefineNuxtComponent {}
class UseFetch {}
class UseState {}
